const app = {
    ...
    renderSkillsUI(mods, metNivel, treino) {
        ...
        container.innerHTML = this.skillsRef.map(s => {
            ...
            const finalMod = mods[s.a] + metNivel + bonus + (data?.bonus || 0);
            return `
    ...
    saveArma() {
        ...
        if (cat === 'Geral') {
            arma = {
                ...
                tipo: "Item",
                cat: "Geral",
                tipoItem: document.getElementById('arma-tipo-item').value || "Item",
                    ...
                    <div class="list-item-box">
                        <div class="list-item-title" style="align-items: center;">
                            ...
                            <div style="display:flex; align-items:center; gap: 10px;">
                                <button class="btn-edit" onclick="app.editarArma(${i})" style="background:var(--accent-light); color:white; border:none; padding:6px 10px; border-radius:4px; font-size:0.7rem; cursor:pointer;">✏️</button>
    ...
    editarInvocacao(idx) {
        console.log('>>> editarInvocacao called with idx:', idx);
        const char = this.characters[this.activeIdx];
        const inv = char.invocacoes[idx];
        
        console.log('>>> Invocacao data:', inv);
        ...
        // Render skills and check selected
    ...
    rollInvocaDano(idx) {
        ...
        // Só usa o dano customizado definido pelo usuário
        if (customDano && customDano.trim() !== '') {
            // Parse do formato de dado (ex: "1d20", "2d6+5", "1d8+CON")
            const dmgStr = customDano.toUpperCase().replace(/CON/g, String(Math.floor((invoca.attrs.CON - 10) / 2)));
            
            // Regex para parse: "XdY+Z" ou "XdY-Z"
            ...
            if (match) {
                ...
                toast.innerHTML = `
                    ...
                    <div class="toast-value" style="color:var(--hp); font-size:2rem;">${total}</div>
                    <div class="toast-math">${numDice}d${faces}${bonus >= 0 ? '+' + bonus : bonus} (${rolls.join(', ')})</div>
                `;
                document.getElementById('toast-container').appendChild(toast);
                setTimeout(() => toast.remove(), 5000);
                return;
            } else {
                // Se não conseguir parsear, mostra erro
                this.showToast('Formato de dano inválido. Use: 1d20, 2d6+5, 1d8+CON', 'error');
                return;
            }
        }
        
        // Se não tiver dano customizado, mostra mensagem para o usuário definir
