const app = {
    ...
    rollInvocaDano(idx) {
        const char = this.characters[this.activeIdx];
        const invoca = char.invocacoes[idx];
        const grau = invoca.grau || 4;
        const modCON = Math.floor((invoca.attrs.CON - 10) / 2);
        
        let dice, faces;
        switch(grau) {
            case 3: dice = 1; faces = 8; break;
            case 2: dice = 1; faces = 10; break;
            case 1: dice = 2; faces = 6; break;
            case 0: dice = 2; faces = 8; break;
            default: dice = 1; faces = 6;
        }
        
        let rollDano = 0;
        for (let i = 0; i < dice; i++) rollDano += Math.floor(Math.random() * faces) + 1;
        
        const dmg = rollDano + modCON;
        const toast = document.createElement('div');
        toast.className = 'dice-toast fade-in';
        toast.style.borderColor = 'var(--hp)';
        toast.innerHTML = `<div class="toast-title">Dano ${invoca.nome}</div><div class="toast-value" style="color:var(--hp); font-size:2rem;">${dmg}</div><div class="toast-math">${dice}d${faces} + CON (${modCON})</div>`;
        document.getElementById('toast-container').appendChild(toast);
