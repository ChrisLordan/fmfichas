const app = {
    ...
    loggedIn: false,
    skillsRef: [
        { n: "Acrobacia", a: "DEX" }, { n: "Furtividade", a: "DEX" }, { n: "Prestidigitação", a: "DEX" },
        ...
        { n: "Persuasão", a: "PRE" }, { n: "Vontade", a: "SAB" }
    ],
    aptitudesRef: [
    ...
    createNewCharWithUID(uid) {
        const skData = {};
        this.skillsRef.forEach(s => skData[s.n] = { trained: false, bonus: 0 });
        ...
        const newChar = {
            ...
            hp: { curr: 12, max: 12 }, pe: { curr: 4, max: 4 },
    ...
    openSheet(idx) {
        ...
        document.getElementById('cd-trained').checked = char.cdEspec.trained;
    ...
    autoCalculate() {
        ...
        const valCd = 10 + mods[char.cdEspec.attr] + (char.cdEspec.trained ? treino : 0) + metNivel + char.cdEspec.var;
        document.getElementById('cd-espec-val').innerText = valCd;

        const perData = char.skills["Percepção"];
        ...
        const valAstucia = mods.INT + metNivel;
        const valFortitude = mods.CON + metNivel;
        const valReflexos = mods.DEX + metNivel;
        const valVontade = mods.SAB + metNivel;
        
        document.getElementById('astucia-val').value = valAstucia;
        document.getElementById('fortitude-val').value = valFortitude;
        document.getElementById('reflexos-val').value = valReflexos;
        document.getElementById('vontade-val').value = valVontade;
        
        if (!char.tr) char.tr = {};
        char.tr.astucia = valAstucia;
        char.tr.fortitude = valFortitude;
        char.tr.reflexos = valReflexos;
        char.tr.vontade = valVontade;
    ...
    updateCdEspecUI() {
        if (this.activeIdx !== null) {
            ...
            char.cdEspec.trained = document.getElementById('cd-trained').checked;
    ...
    renderSkillsUI(mods, halfLvl, treino) {
        ...
        container.innerHTML = this.skillsRef.map(s => {
            ...
            const finalMod = mods[s.a] + halfLvl + (data.trained ? treino : 0) + data.bonus;
            return `
            <div class="skill-item">
                <label>
                    <input type="checkbox" class="custom-checkbox" ${data.trained ? 'checked' : ''} onchange="app.toggleSkill('${s.n}', this.checked)">
    ...
    toggleSkill(name, state) {
        this.characters[this.activeIdx].skills[name].trained = state;
        this.autoCalculate();
    },
    ...
    renderInventario() {
        if (this.activeIdx === null) return;
        const char = this.characters[this.activeIdx];
        const html = char.inventario.map((arma, i) => `
            <div class="list-item-box">
                <div class="list-item-title" style="align-items: center;">
                    <span>${arma.nome} <small style="color:#888; font-weight:normal; font-size:0.7rem; display:block;">${arma.tipo} - ${arma.cat}</small></span>
                ...
                <div class="list-item-desc" style="margin-top: 8px;">
                    ${arma.desc ? `<i>"${arma.desc}"</i><br>` : ''}
                    <strong>Dano:</strong> ${arma.dano} | <strong>Atributo:</strong> ${arma.cat === 'Corpo a corpo' ? arma.attr : 'N/A'} <br>
                    <strong style="color:var(--accent)">Variáveis:</strong> Ataque ${arma.varAtk >= 0 ? '+' + arma.varAtk : arma.varAtk} / Dano ${arma.varDano >= 0 ? '+' + arma.varDano : arma.varDano}
                </div>
            </div>
        `).join('');
    ...
    rollFeitico(feat) {
        ...
        const attrVal = this.characters[this.activeIdx].attrs[attr] || 10;
        ...
        document.getElementById('apt-perfil-list').innerHTML = char.perfilAmaldicado.aptidoes.map((apt, i) => `
            <div style="background:var(--input); padding:10px; border-radius:4px; margin-bottom:10px; border:1px solid var(--border);">
                ...
                <div style="display:flex; align-items:center; gap:10px; margin-top:8px;">
                    <span style="font-size:0.75rem; color:#888;">Nível:</span>
                    <input type="number" min="0" max="5" value="${apt.nivel || 0}" 
                        style="width:40px; background:var(--bg); border:1px solid var(--border); color:var(--text); border-radius:3px; padding:3px;"
                        onchange="app.updateAptNivel(${i}, this.value)">
                </div>
            </div>
        `).join('');
