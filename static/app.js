const app = {
    ...
    openSheet(idx) {
        ...
        document.getElementById('cd-feiticos-perfil').value = char.perfilAmaldicado.cdFeiticos || 10;
    ...
    autoCalculate() {
        ...
        const ccTot = mods[char.combat.cc.attr] + metNivel + (char.combat.cc.trained ? treino : 0) + (char.combat.cc.bonus || 0);
        document.getElementById('cc-total').innerText = ccTot >= 0 ? '+' + ccTot : ccTot;
        char.combat.cc.total = ccTot;

        const distTot = mods.DEX + metNivel + (char.combat.dist.trained ? treino : 0) + (char.combat.dist.bonus || 0);
    ...
    renderSkillsUI(mods, metNivel, treino) {
        ...
        container.innerHTML = this.skillsRef.map(s => {
            ...
            const finalMod = mods[s.a] + metNivel + bonus + (data?.bonus || 0) + versatilidade;
            return `
    ...
    rollDice(name, mod, bonusMsg = "") {
        ...
        if (this.vantagemAtiva) {
            ...
            d20 = Math.max(d1, d2);
            usedD20 = `VANTAGEM: ${d1} / ${d2} → ${d20}`;
        } else if (this.desvantagemAtiva) {
            const d1 = Math.floor(Math.random() * 20) + 1;
            const d2 = Math.floor(Math.random() * 20) + 1;
            d20 = Math.min(d1, d2);
            usedD20 = `DESVANTAGEM: ${d1} / ${d2} → ${d20}`;
        } else {
            ...
            if (hasDano) {
                if (tipo === 'ambos') {
                    buttons = `<div style="display:flex; gap:8px; margin:5px 0;">
                        <button class="btn-roll" onclick="app.rollFeitico('${f.replace(/'/g, "\\'")}', 'ataque')">⚔️ Atk ${dano}</button>
                        <button class="btn-roll" style="background:var(--accent);" onclick="app.rollFeitico('${f.replace(/'/g, "\\'")}', 'cd')">🎯 CD ${dano}</button>
                    </div>`;
                } else if (tipo === 'cd') {
                    buttons = `<div style="display:flex; gap:8px; margin:5px 0;">
                        <button class="btn-roll" style="background:var(--accent);" onclick="app.rollFeitico('${f.replace(/'/g, "\\'")}', 'cd')">🎯 Rolar CD ${dano}</button>
                    </div>`;
                } else {
                    buttons = `<div style="display:flex; gap:8px; margin:5px 0;">
                        <button class="btn-roll" onclick="app.rollFeitico('${f.replace(/'/g, "\\'")}', 'ataque')">⚔️ Rolar ${dano}</button>
                    </div>`;
                }
            }
...
document.getElementById('hab-edit-idx').value = idx;
    ...
    rollFeitico(feat, tipo = 'ataque') {
        ...
        if (result.total === undefined || isNaN(result.total)) {
        ...
        if (testeTipo === 'cd') {
            ...
            toast.innerHTML = `<div class="toast-title">${nome} (CD)</div><div class="toast-value" style="color:var(--accent); font-size:2rem;">${result.total}</div><div class="toast-math">${dano}</div>`;
        ...
        toast.innerHTML = `<div class="toast-title">${nome}</div><div class="toast-value" style="color:${d20 === 20 ? '#22c55e' : (d20 === 1 ? 'var(--hp)' : 'var(--text)')};">${total}</div><div class="toast-math">D20 (${d20}) ${signMod >= 0 ? '+' : ''}${signMod}</div><div class="toast-math">${dano} (${result.rolls.join(', ')})</div>`;
    ...
    savePerfilCDs() {
        ...
        char.perfilAmaldicado.cdFeiticos = parseInt(document.getElementById('cd-feiticos-perfil').value) || 10;
        this.saveData();
    },
    ...
    updateCondicoes() {
        ...
        // Calcular penalidades combinadas (peor de cada tipo)
        let efeitos = [];
        
        // Abalado (Fraca): -1 em ataques
        if (condicoes.abalado) {
            efeitos.push("-1 Ataque");
        }
        
        // Exposto (Fraca): -2 em testes
        if (condicoes.exposto) {
            efeitos.push("-2 Testes");
        }
        
        // Amedrontado (Média): -3 em ataques e testes
        if (condicoes.amedrontado) {
            efeitos.push("-3 Ataque/Testes");
        }
        
        // Enredado (Média): -2 em testes de desarme
        if (condicoes.enredado) {
            efeitos.push("-2 Desarme");
        }
        
        // Lento (Média): -2 em testes e ataques
        if (condicoes.lento) {
            efeitos.push("-2 Testes/Ataque");
        }
        
        // Cego (Forte): -5 em ataques, falha percepção
        if (condicoes.cego) {
            efeitos.push("-5 Ataque, Percepção com desvantagem");
        }
        
        // Imóvel (Forte): não pode se mover
        if (condicoes.imvel) {
            efeitos.push("Imóvel (não pode mover)");
        }
    ...
    renderShikigamisResumo() {
        ...
        container.innerHTML = char.invocacoes.map((inv, i) => {
            ...
            return `<div style="display:flex; justify-content:space-between; align-items:center; padding:8px; background:var(--input); border-radius:4px; margin-bottom:5px; border:1px solid var(--border);">
                <div style="flex:1;">
                ...
                <div style="display:flex; gap:10px; align-items:center;">
                    ...
                    <button class="btn-roll" style="padding:4px 8px; font-size:0.7rem;" onclick="app.rollInvocaAtk(${i}, 'FOR')">ATK</button>
                    <button class="btn-roll" style="background:var(--hp); padding:4px 8px; font-size:0.7rem;" onclick="app.rollInvocaDano(${i})">Dano</button>
                </div>
            </div>`;
        }).join('');
    },
    ...
    openShikigamiPanel() {
        ...
        container.innerHTML = char.invocacoes.map((inv, i) => {
            const grau = grauNome[inv.grau] || '4º';
            return `
                <div style="background: var(--input); padding: 12px; border-radius: 8px; margin-bottom: 10px; border: 1px solid var(--border);">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                        <div>
                    ...
                    <div style="display: flex; gap: 5px; flex-wrap: wrap;">
                        <button class="btn-roll" onclick="app.rollInvocaAtk(${i}, 'FOR')">ATK FOR</button>
                        <button class="btn-roll" onclick="app.rollInvocaAtk(${i}, 'DEX')">ATK DEX</button>
                        <button class="btn-roll" style="background: var(--hp);" onclick="app.rollInvocaDano(${i})">Dano</button>
                        <button class="btn-roll" style="background: var(--accent);" onclick="app.rollInvocaCD(${i})">Testar CD</button>
                    </div>
                    
                    <div style="display: flex; gap: 5px; margin-top: 8px;">
                        <button class="btn-ctrl" onclick="app.adjustInvocaPV(${i}, -5)" style="padding: 4px 8px; font-size: 0.7rem;">-5</button>
                        <button class="btn-ctrl" onclick="app.adjustInvocaPV(${i}, -1)" style="padding: 4px 8px; font-size: 0.7rem;">-1</button>
                        <button class="btn-ctrl" onclick="app.adjustInvocaPV(${i}, 1)" style="padding: 4px 8px; font-size: 0.7rem;">+1</button>
                        <button class="btn-ctrl" onclick="app.adjustInvocaPV(${i}, 5)" style="padding: 4px 8px; font-size: 0.7rem;">+5</button>
                    </div>
                </div>
    ...
    renderInvocacoes() {
        ...
        container.innerHTML = char.invocacoes.map((inv, i) => {
            ...
            const grauNome = ['', '1º Grau', '2º Grau', '3º Grau', '4º Grau', 'Grau Especial'][inv.grau] || '4º Grau';
            const cd = inv.cd || 10;
            
            return `<div class="card" style="margin-bottom:15px; padding:15px;">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px;">
                    <span style="color:var(--accent); font-weight:bold; font-size:1.1rem;">${inv.nome} <span style="color:var(--hp); font-size:0.8rem;">(${grauNome})</span></span>
                    <div style="display:flex; gap:5px;">
                        <button class="btn-edit" onclick="app.editarInvocacao(${i})" style="background:var(--accent-light); color:white; border:none; padding:6px 10px; border-radius:4px; font-size:0.7rem; cursor:pointer;">✏️</button>
                ...
                <div style="display:flex; gap:15px; margin-bottom:10px; flex-wrap:wrap;">
                    ...
                    ${inv.trTreinado ? `<div style="background:var(--accent); color:white; padding:4px 8px; border-radius:4px; font-size:0.8rem;">TR: ${inv.trTreinado}</div>` : ''}
                </div>
                
                ${(inv.acoes && inv.acoes.length > 0) || (inv.carac && inv.carac.length > 0) ? `
                <div style="margin-bottom:10px; padding:8px; background:var(--input); border-radius:4px; border:1px solid var(--border);">
                    ${inv.acoes && inv.acoes.length > 0 ? `<div style="margin-bottom:5px;"><span style="font-size:0.7rem; color:#888; font-weight:bold;">AÇÕES:</span><span style="font-size:0.75rem; color:var(--text); margin-left:5px;">${inv.acoes.join(', ')}</span></div>` : ''}
                    ${inv.carac && inv.carac.length > 0 ? `<div><span style="font-size:0.7rem; color:#888; font-weight:bold;">CARAC:</span><span style="font-size:0.75rem; color:var(--text); margin-left:5px;">${inv.carac.join(', ')}</span></div>` : ''}
                </div>
                ` : ''}
                
                ${inv.pericias && inv.pericias.length > 0 ? `
                <div class="section-title" style="font-size:0.7rem; margin-bottom:8px;"><span>Perícias Treinadas (${inv.pericias.length})</span></div>
                <div style="display:flex; flex-wrap:wrap; gap:5px; margin-bottom:10px;">
                    ${inv.pericias.map(p => {
                        ...
                        return `<button class="btn-roll" style="font-size:0.7rem; padding:4px 8px;" onclick="app.rollInvocaPericia(${i}, '${p}')">${p} (${mod >= 0 ? '+' : ''}${mod})</button>`;
                    }).join('')}
                </div>
                ` : ''}
                ...
                <div style="display:flex; gap:8px; flex-wrap:wrap; margin-top:10px; padding-top:10px; border-top:1px solid var(--border);">
                    <button class="btn-roll" onclick="app.rollInvocaAtk(${i}, 'FOR')">ATK FOR</button>
                    <button class="btn-roll" onclick="app.rollInvocaAtk(${i}, 'DEX')">ATK DEX</button>
                    <button class="btn-roll" onclick="app.rollInvocaAtk(${i}, 'CON')">ATK CON</button>
                    <button class="btn-roll" onclick="app.rollInvocaAtk(${i}, 'INT')">ATK INT</button>
                    <button class="btn-roll" onclick="app.rollInvocaAtk(${i}, 'SAB')">ATK SAB</button>
                    <button class="btn-roll" onclick="app.rollInvocaAtk(${i}, 'PRE')">ATK PRE</button>
                    <button class="btn-roll" style="background:var(--hp);" onclick="app.rollInvocaDano(${i})">Dano</button>
                    <button class="btn-roll" style="background:var(--accent);" onclick="app.rollInvocaCD(${i})">Testar CD</button>
                </div>
            </div>`;
...
app.renderizarFichasCampanha = function(campanhaId) {
    console.log('>> Renderizando fichas da campanha:', campanhaId);
