const app = {
    ...
    specStats: {
        ...
        "Especialista em Técnica": { hp: 10, pe: 6, dv: "d8", hpOpcao: 5, peAttr: "INT", peAttrOptions: ["INT", "SAB"] },
        "Controlador": { hp: 10, pe: 5, dv: "d8", hpOpcao: 5, peAttr: "SAB", peAttrOptions: ["SAB", "INT"] },
        "Suporte": { hp: 10, pe: 5, dv: "d8", hpOpcao: 5, peAttr: "SAB", peAttrOptions: ["SAB", "PRE"] },
    ...
    saveEditSaldo() {
        ...
        const confirmMsg = confirm('Definir saldo para R$ ' + newSaldo.toFixed(2) + '?');
        ...
        this.showToast('Saldo atualizado!', 'success');
        btn.disabled = false;
                    ...
                    if (idx !== -1) {
                        ...
                        if (this.activeIdx === idx && saldoServidor !== saldoAnterior) {
                            ...
                            if (saldoServidor > saldoAnterior) {
                                this.showToast(`Recebido: ${this.formatMoney(saldoServidor - saldoAnterior)}`, 'pix');
                            }
                        }
                        this.saveData();
                    }
        ...
        setInterval(() => {
            ...
            db.ref('saldos/' + char.pix.uid).once('value').then(snapshot => {
                ...
                if (saldoServidor !== saldoLocal) {
                    ...
                    this.updateSaldoDisplay();
                    if (saldoServidor > saldoLocal) {
                        this.showToast(`Recebido: ${this.formatMoney(saldoServidor - saldoLocal)}`, 'pix');
                    }
                    this.saveData();
                }
            }).catch(() => {});
    ...
    generateNewUID() {
        ...
        this.showToast('UID atualizado!', 'success');
    },
    ...
    processTransfer() {
        ...
        if (!uidDestino || uidDestino.length !== 6) {
            return this.showToast('UID inválido. Use 6 dígitos.', 'error');
        }
        if (valor <= 0) {
            return this.showToast('Valor deve ser maior que zero.', 'error');
        }
        if (charRemetente.pix.uid === uidDestino) {
            return this.showToast('Não pode transferir para si mesmo.', 'error');
        }
        ...
        .then(() => {
            ...
            document.getElementById('transfer-uid').value = '';
            document.getElementById('transfer-valor').value = '';
            this.showToast('Enviado: R$ ' + valor.toFixed(2), 'success');
        })
        .catch(err => {
            this.showToast(err.message || 'Erro na transferência', 'error');
        })
        .finally(() => {
            btn.disabled = false;
    ...
    showNotifications() {
        this.showToast('Veja o saldo em tempo real!', 'info');
    },

    login(uid) {
        if (!uid || uid.length !== 6) {
            return this.showToast('UID inválido', 'error');
        }
        ...
        if (existingIdx !== -1) {
            ...
            return this.showToast('Login realizado!', 'success');
        }
        
        if (this.characters.length >= 15) {
            return this.showToast('Limite de 15 fichas', 'error');
        }
    ...
    createNewCharWithUID(uid) {
        ...
        const newChar = {
            ...
            hp: { curr: 12, max: 12, temp: 0 }, pe: { curr: 4, max: 4, temp: 0 }, integridade: { curr: 12, max: 12 },
            skills: skData, aptitudes: {},
            habilidades: [], feiticos: [], talentos: [], inventario: [],
            invocacoes: [], condicoes: [],
            ...
            pix: { saldo: 0, uid: uid + '_' + this.generateUID(), transferencias: [] }
        };
        const stats = this.levelRewards[newChar.spec];
        if (stats && stats.peAttr) {
            newChar.peAttr = stats.peAttr;
        }
        ...
        this.saveData();
        this.showToast('Conta criada automaticamente!', 'success');
    },
    ...
    criarConta() {
        ...
        db.ref('usuarios/' + uid).set({
            ...
            this.showToast('Conta criada! UID: ' + uid, 'success');
    ...
    fazerLogin() {
        ...
        if (!nome || !senha) {
            this.showToast('Preencha nome e senha!', 'error');
            return;
        }
        ...
        if (uidInput) {
            db.ref('usuarios/' + uidInput).once('value').then(snapshot => {
                ...
                if (usuario && usuario.nome === nome && usuario.senha === senha) {
                    ...
                    this.showToast('UID, nome ou senha incorretos', 'error');
                }
            }).catch(() => {
            ...
            db.ref('usuarios').orderByChild('nome').equalTo(nome).once('value').then(snapshot => {
                ...
                if (usuario) {
                    ...
                    this.showToast('Nome ou senha incorretos', 'error');
                }
            }).catch(() => {
                this.showToast('Erro ao fazer login', 'error');
            });
    ...
    carregarAvatarConta() {
        ...
        const cachedAvatar = localStorage.getItem('jjk_avatar_' + this.userUid);
        const avatars = document.querySelectorAll('#conta-avatar, #modal-conta-avatar');
        if (cachedAvatar) {
            avatars.forEach(img => {
        ...
        db.ref('usuarios/' + this.userUid + '/avatar').once('value').then(snapshot => {
            ...
            if (avatar) {
                ...
                localStorage.setItem('jjk_avatar_' + this.userUid, avatar);
                avatars.forEach(img => {
                    if (img) img.src = avatar;
                });
            }
    ...
    iniciarEscutaFichasFirebase(uid) {
        if (!uid) return;
        
        this.fichaFirebaseListeners = true;
        this.fichaFirebaseUid = uid;
        
        if (this.fichaPollingInterval) {
            clearInterval(this.fichaPollingInterval);
        }
        
        const self = this;
        this.fichaPollingInterval = setInterval(() => {
            if (!self.activeIdx || !self.characters || !self.characters[self.activeIdx]) return;
            
            const uid = self.fichaFirebaseUid;
            if (!uid) return;
            ...
            db.ref('usuarios/' + uid + '/fichas').once('value').then(snapshot => {
                ...
                const idx = self.activeIdx;
                
                if (fichasData && fichasData[idx]) {
                    const charData = fichasData[idx];
                    const localChar = self.characters[idx];
                    
                    if (!localChar || !charData) return;
                    
                    let mudou = false;
                    
                    if (localChar.hp?.curr !== charData.hp?.curr || localChar.hp?.max !== charData.hp?.max) mudou = true;
                    if (localChar.pe?.curr !== charData.pe?.curr || localChar.pe?.max !== charData.pe?.max) mudou = true;
                    if (localChar.name !== charData.name) mudou = true;
                    if (localChar.lvl !== charData.lvl) mudou = true;
                    if (localChar.hp?.temp !== charData.hp?.temp) mudou = true;
                    if (localChar.pe?.temp !== charData.pe?.temp) mudou = true;
                    
                    if (mudou) {
                        self.characters[idx] = charData;
    ...
    pararEscutaFichasFirebase() {
        if (this.fichaPollingInterval) {
            clearInterval(this.fichaPollingInterval);
            this.fichaPollingInterval = null;
        }
        this.fichaFirebaseListeners = false;
    },
        ...
        return new Promise((resolve, reject) => {
            ...
            db.ref('usuarios/' + uid + '/fichas').once('value').then(snapshot => {
                ...
                if (this.characters && this.characters.length > 0) {
                    this.iniciarEscutaFichasFirebase(uid);
    ...
    editarFotoConta(e) {
        ...
        reader.onload = ev => {
            document.getElementById('foto-tipo-edicao').value = 'conta';
            document.getElementById('foto-temp-data').value = ev.target.result;
            document.getElementById('foto-zoom').value = 1;
            document.getElementById('foto-tamanho').value = 100;
            document.getElementById('foto-preview-img').src = ev.target.result;
            ui.openModal('modal-foto-ajuste');
        };
    ...
    previewFotoAjuste() {
        ...
        const zoom = parseFloat(document.getElementById('foto-zoom').value);
        const tamanho = parseInt(document.getElementById('foto-tamanho').value);
        
        img.style.transform = `scale(${zoom})`;
        
        const container = document.getElementById('foto-preview-container');
        const baseSize = 200;
        const newSize = (baseSize * tamanho) / 100;
    ...
    confirmarFotoAjuste() {
        const tipo = document.getElementById('foto-tipo-edicao').value;
        const tempData = document.getElementById('foto-temp-data').value;
        
        if (tipo === 'conta') {
            document.getElementById('conta-avatar').src = tempData;
            document.getElementById('modal-conta-avatar').src = tempData;
            
            db.ref('usuarios/' + this.userUid + '/avatar').set(tempData).then(() => {
                this.showToast('Foto atualizada!', 'success');
            }).catch(() => {
                this.showToast('Erro ao salvar foto', 'error');
            });
        } else if (tipo === 'ficha') {
            document.getElementById('char-avatar').src = tempData;
            this.characters[this.activeIdx].avatar = tempData;
            this.saveData();
            this.renderCharList();
            this.showToast('Foto atualizada!', 'success');
        }
        
        ui.closeModal('modal-foto-ajuste');
    },
    ...
    createNewChar() {
        ...
        const newChar = {
            ...
            pix: { saldo: 0, uid: this.userUid + '_' + this.generateUID(), transferencias: [] }
        };
        const stats = this.levelRewards[newChar.spec];
        if (stats && stats.peAttr) {
            newChar.peAttr = stats.peAttr;
    ...
    openSheet(idx) {
        try {
            ...
            if (!char.condicaoMods) char.condicaoMods = { atk: 0, pericia: 0, def: 0, tr: 0 };
            if (!char.hp) char.hp = { curr: 12, max: 12, temp: 0 };
            if (!char.hp.temp) char.hp.temp = 0;
            if (!char.pe) char.pe = { curr: 4, max: 4, temp: 0 };
            if (!char.pe.temp) char.pe.temp = 0;
            if (!char.integridade) char.integridade = { curr: 12, max: 12 };
        ...
        document.getElementById('cd-tecnica-perfil').value = char.perfilAmaldicado.cdTecnica || 10;
        document.getElementById('cd-amaldicada-perfil').value = char.perfilAmaldicado.cdAmaldicada || 10;
        document.getElementById('cd-especializacao-perfil').value = char.perfilAmaldicado.cdEspecializacao || 10;
    ...
    autoCalculate() {
        ...
        const valDef = 10 + mods.DEX + metNivel + char.defesaExtras.eqp + char.defesaExtras.var + defBonus;
        document.getElementById('def-val').innerText = valDef;
        document.getElementById('treino-val').innerText = "+" + treino;

        const valCd = 10 + metNivel + mods[char.cdEspec.attr] + treino + char.cdEspec.var;
        document.getElementById('cd-espec-val').innerText = valCd;

        const perData = char.skills["Percepção"] || { trained: false, bonus: 0 };
        const valAten = 10 + metNivel + mods.SAB + (perData?.trained ? treino : 0) + (perData?.bonus || 0);
        ...
        ['astucia', 'fortitude', 'reflexos', 'vontade', 'integridade'].forEach(tr => {
            if (!char.trData[tr]) char.trData[tr] = { trained: false, master: false, var: 0 };
            const trData = char.trData[tr];
            const bonus = trData.trained ? trainedBonus : (trData.master ? masterBonus : 0);
            
            // Aplicar modificadores de condições
            const condMods = char.condicaoMods || { atk: 0, pericia: 0, def: 0, tr: 0 };
            char.tr[tr] = baseTR[tr] + metNivel + bonus + (trData.var || 0) + (condMods.tr || 0);
        ...
        if (!char.combat) char.combat = { cc: { attr: 'FOR', trained: false, bonus: 0 }, dist: { attr: 'DEX', trained: false, bonus: 0 }, amal: { attr: 'INT', trained: false, bonus: 0 } };
        if (!char.combat.cc) char.combat.cc = { attr: 'FOR', trained: false, bonus: 0 };
        if (!char.combat.dist) char.combat.dist = { attr: 'DEX', trained: false, bonus: 0 };
        if (!char.combat.amal) char.combat.amal = { attr: 'INT', trained: false, bonus: 0 };
        
        const condMods = char.condicaoMods || { atk: 0, pericia: 0, def: 0, tr: 0 };
        
        const ccTot = mods[char.combat.cc.attr] + metNivel + (char.combat.cc.trained ? treino : 0) + (char.combat.cc.bonus || 0) + (condMods.atk || 0);
        document.getElementById('cc-total').innerText = ccTot >= 0 ? '+' + ccTot : ccTot;
        char.combat.cc.total = ccTot;

        const distTot = mods.DEX + metNivel + (char.combat.dist.trained ? treino : 0) + (char.combat.dist.bonus || 0) + (condMods.atk || 0);
        document.getElementById('dist-total').innerText = distTot >= 0 ? '+' + distTot : distTot;
        char.combat.dist.total = distTot;

        const AmalTot = mods[char.combat.amal.attr] + metNivel + (char.combat.amal.trained ? treino : 0) + (char.combat.amal.bonus || 0);
        ...
        if (char.iniciativa !== initMod) {
            char.iniciativa = initMod;
            document.getElementById('iniciativa-val').value = initMod;
        }
        ...
        // PE cálculo: basePe * nivel + modificador (não multiplica o modificador por nível)
        const basePe = stats.pe;
        let peBonusAttr = 0;
        const peAttrToUse = char.peAttr || stats.peAttr;
        if (peAttrToUse) {
            peBonusAttr = this.getModifier(char.attrs[peAttrToUse]) || 0;
        }
        char.pe.max = basePe * char.lvl + peBonusAttr;
    ...
    updateCdEspecUI() {
        if (this.activeIdx !== null) {
            ...
            const valCd = 10 + char.lvl + mods[char.cdEspec.attr] + treino + char.cdEspec.var;
    ...
    applyLevelUp(oldLvl, newLvl) {
        ...
        // PE: basePe * nivel + modificador (não multiplica o modificador por nível)
        let basePe = stats.pe;
        let peBonusAttr = 0;
        const peAttrToUse = char.peAttr || stats.peAttr;
        if (peAttrToUse) {
        ...
        char.pe.max = newLvl * basePe + peBonusAttr;
    ...
    rollTR(type) {
        ...
        // Usar o valor que já foi calculado pelo autoCalculate
        const valorTR = parseInt(document.getElementById(trKey + '-val').value) || 0;
        
        this.rollDice(type, valorTR);
    },
    ...
    adjustResource(type, amount) {
        ...
        if (type === 'hp') {
            ...
            if (amount < 0) {
                const remainingAfterTemp = temp + amount;
                if (remainingAfterTemp >= 0) {
                    char.hp.temp = remainingAfterTemp;
                } else {
                    char.hp.temp = 0;
                    char.hp.curr += remainingAfterTemp;
                }
            } else {
                char.hp.curr += amount;
            }
            if (char.hp.curr > char.hp.max + temp) char.hp.curr = char.hp.max + temp;
        } else if (type === 'pe') {
            const temp = char.pe.temp || 0;
            if (amount < 0) {
                const remainingAfterTemp = temp + amount;
                if (remainingAfterTemp >= 0) {
                    char.pe.temp = remainingAfterTemp;
                } else {
                    char.pe.temp = 0;
                    char.pe.curr += remainingAfterTemp;
                }
            } else {
                char.pe.curr += amount;
            }
            if (char.pe.curr > char.pe.max + temp) char.pe.curr = char.pe.max + temp;
        } else {
    ...
    updateTempResource(type) {
        const char = this.characters[this.activeIdx];
        const tempInput = document.getElementById(type + '-temp');
        const maxTemp = Math.floor(char[type].max / 2);
        let temp = parseInt(tempInput.value) || 0;
        ...
        if (!char[type].temp) char[type].temp = 0;
        char[type].temp = temp;
        this.updateResourceUI();
        this.saveData();
    },
    ...
    changePeAttr(attr) {
        const char = this.characters[this.activeIdx];
        const stats = this.levelRewards[char.spec];
        if (!stats || !stats.peAttrOptions) return;
        if (!stats.peAttrOptions.includes(attr)) {
            attr = stats.peAttr;
        }
        char.peAttr = attr;
        this.autoCalculate();
    ...
    updateResourceUI() {
        ...
        const hpTemp = char.hp.temp || 0;
        const peTemp = char.pe.temp || 0;
        
        document.getElementById('curr-hp').innerText = char.hp.curr;
        document.getElementById('max-hp').innerText = char.hp.max;
        document.getElementById('hp-temp-display').innerText = hpTemp > 0 ? ` + ${hpTemp}` : '';
        document.getElementById('hp-temp').value = hpTemp;
        
        document.getElementById('curr-pe').innerText = char.pe.curr;
        document.getElementById('max-pe').innerText = char.pe.max;
        document.getElementById('pe-temp').value = peTemp;
        
        const peAttrSelect = document.getElementById('pe-attr-select');
        const peAttrConfig = document.getElementById('pe-attr-config');
        const stats = this.levelRewards[char.spec];
        if (peAttrSelect && stats && stats.peAttrOptions) {
            peAttrSelect.innerHTML = stats.peAttrOptions.map(a => `<option value="${a}">${a}</option>`).join('');
            const currentAttr = char.peAttr || stats.peAttr;
            if (currentAttr && stats.peAttrOptions.includes(currentAttr)) {
                peAttrSelect.value = currentAttr;
            }
        }
        if (peAttrConfig) {
            peAttrConfig.style.display = (stats && stats.peAttrOptions && stats.peAttrOptions.length > 1) ? 'flex' : 'none';
        }
        ...
        const pctHp = char.hp.max > 0 ? (char.hp.curr / (char.hp.max + hpTemp) * 100) : 0;
        const pctPe = char.pe.max > 0 ? (char.pe.curr / (char.pe.max + peTemp) * 100) : 0;
    ...
    renderSkillsUI(mods, metNivel, treino) {
        const char = this.characters[this.activeIdx];
        ...
        // Versatilidade para Restringido (Técnicas Marciais)
        let versatilidade = 0;
        if (char.origin === 'Restringido') {
            if (char.lvl >= 16) versatilidade = 3;
            else if (char.lvl >= 8) versatilidade = 2;
            else if (char.lvl >= 5) versatilidade = 1;
        }
        ...
        container.innerHTML = this.skillsRef.map(s => {
            ...
            const condPericia = condMods.pericia || 0;
            const finalMod = mods[s.a] + metNivel + bonus + (data?.bonus || 0) + versatilidade + condPericia;
            return `
            <div class="skill-item">
                <label style="display:flex; align-items:center; gap:5px;">
                    ...
                    ${versatilidade > 0 ? `<span style="font-size:0.6rem; color:var(--accent); margin-left:3px;">(+${versatilidade} Vers)</span>` : ''}
                </label>
    ...
    rollCombat(type) {
        ...
        const condMods = char.condicaoMods || { atk: 0, pericia: 0, def: 0, tr: 0 };
        const finalTotal = total + bonusAtk + (condMods.atk || 0);
        ...
        if (condMods.atk !== 0) {
            bonusMsg += `<br><small style="color:var(--hp)">Condições: ${condMods.atk > 0 ? '+' : ''}${condMods.atk}</small>`;
        }
    ...
    toggleVantagem() {
        this.vantagemAtiva = !this.vantagemAtiva;
        this.desvantagemAtiva = false;
        document.getElementById('btn-vantagem').style.background = this.vantagemAtiva ? 'var(--accent)' : 'var(--border)';
        document.getElementById('btn-vantagem').style.color = this.vantagemAtiva ? 'white' : 'var(--text)';
        document.getElementById('btn-desvantagem').style.background = 'var(--border)';
        document.getElementById('btn-desvantagem').style.color = 'var(--text)';
    },

    toggleDesvantagem() {
        this.desvantagemAtiva = !this.desvantagemAtiva;
        this.vantagemAtiva = false;
        document.getElementById('btn-desvantagem').style.background = this.desvantagemAtiva ? 'var(--hp)' : 'var(--border)';
        document.getElementById('btn-desvantagem').style.color = this.desvantagemAtiva ? 'white' : 'var(--text)';
        document.getElementById('btn-vantagem').style.background = 'var(--border)';
        document.getElementById('btn-vantagem').style.color = 'var(--text)';
    },

    rollDice(name, mod, bonusMsg = "") {
        let d20, total, usedD20;
        
        // Se ambos estiverem ativos, anulam (roll normal)
        const temVantagem = this.vantagemAtiva;
        const temDesvantagem = this.desvantagemAtiva;
        
        if (temVantagem && temDesvantagem) {
            // Anulam - roll normal
            d20 = Math.floor(Math.random() * 20) + 1;
            usedD20 = `ANULADO: D20 (${d20})`;
        } else if (temVantagem) {
            const d1 = Math.floor(Math.random() * 20) + 1;
            const d20_2 = Math.floor(Math.random() * 20) + 1;
            d20 = Math.max(d1, d20_2);
            usedD20 = `VANTAGEM: ${d1} / ${d20_2} → ${d20}`;
        } else if (temDesvantagem) {
            const d1 = Math.floor(Math.random() * 20) + 1;
            const d20_2 = Math.floor(Math.random() * 20) + 1;
            d20 = Math.min(d1, d20_2);
            usedD20 = `DESVANTAGEM: ${d1} / ${d20_2} → ${d20}`;
        } else {
            d20 = Math.floor(Math.random() * 20) + 1;
            usedD20 = `D20 (${d20})`;
        }
        ...
        // Mostrar status de vantagem/desvantagem
        let statusMsg = '';
        if (temVantagem && temDesvantagem) statusMsg = ' ⚖️ ANULADO';
        else if (temVantagem) statusMsg = ' ▲ VANTAGEM';
        else if (temDesvantagem) statusMsg = ' ▼ DESVANTAGEM';
        ...
        if (this.vantagemAtiva) {
            if (d20 === 20) { color = "var(--accent)"; totalColor = "#22c55e"; }
            else if (d20 === 1) { color = "#f59e0b"; totalColor = "#f59e0b"; }
        } else if (this.desvantagemAtiva) {
            if (d20 === 20) { color = "#f59e0b"; totalColor = "#f59e0b"; }
            else if (d20 === 1) { color = "var(--hp)"; totalColor = "var(--hp)"; }
        } else {
            if (d20 === 20) { color = "var(--accent)"; totalColor = "#22c55e"; }
            if (d20 === 1) { color = "var(--hp)"; totalColor = "var(--hp)"; }
        }

        toast.style.borderColor = this.desvantagemAtiva ? (d20 === 1 ? 'var(--hp)' : '#f59e0b') : (d20 === 20 ? 'var(--accent)' : (d20 === 1 ? 'var(--hp)' : 'var(--border)'));
        if (d20 === 1 && !this.desvantagemAtiva) toast.style.borderLeftColor = "var(--hp)";
        if (d20 === 1 && this.desvantagemAtiva) toast.style.borderLeftColor = "var(--hp)";

        toast.innerHTML = `
            <div class="toast-title">${name}${statusMsg}</div>
            <div class="toast-value" style="color: ${totalColor};">${total}</div>
            <div class="toast-math">${usedD20} ${signMod}${bonusMsg}</div>
        `;
    ...
    editarArma(idx) {
        ...
        document.getElementById('arma-nome').value = arma.nome || "";
        document.getElementById('arma-desc').value = arma.desc || "";
        document.getElementById('arma-tipo').value = arma.tipo || "Arma";
        document.getElementById('arma-cat').value = arma.cat || "Corpo a corpo";
        
        if (arma.cat === 'Geral') {
            document.getElementById('arma-tipo-item').value = arma.tipoItem || "";
            document.getElementById('arma-qtd').value = arma.qtd || 1;
            document.getElementById('arma-efeitos').value = arma.efeitos || "";
        } else {
            document.getElementById('arma-dano').value = arma.dano || "1d6";
            document.getElementById('arma-attr').value = arma.attr || "FOR";
            document.getElementById('arma-var-atk').value = arma.varAtk || 0;
            document.getElementById('arma-var-dano').value = arma.varDano || 0;
        }
        
        document.getElementById('arma-current-edit').value = idx;
        ui.openModal('modal-arma');
    },

    propriedadesArmas: {
        'Agile': 'Armas ágeis podem ser usadas como ação bônus para fazer um ataque adicional após uma ação de movimento.',
        'Attachment': 'A arma pode ser conectada a outras armas ou equipamentos, permitindo combinações.',
        'Beretta': 'Armas de fogo automáticas que podem disparar rajadas.',
        'Dançarina': 'Permite ataques danificados que causam dano extra quando você se move.',
        'Distância': 'A arma pode ser usada para ataques a distância. Tem um alcance diferente.',
        'Dois-pontos': 'Requer duas mãos para ser usada corretamente.',
        'Furtiva': 'Não causa penalidade por ataque furtivo e não awake atenção.',
        'Legendra': 'Uma arma lendária tem propriedades especiais determinadas pelo Mestre.',
        'Macica': 'Pode ser usada com uma mão como ação bônus para golpear com o cabo.',
        'Mortifera': 'A arma causa +1d6 de dano em um crítico.',
        'Paralizante': 'Pode causar a condição Enredado ou Paralisado em certas situações.',
        'Precisa': 'Pode usar seu modificador de Destreza em vez de Força para ataque e dano.',
        'Recarregável': 'Precisa ser recarregada após um número específico de ataques.',
        'Soltável': 'Pode ser solta e recuperada como parte de uma mesma ação.',
        'Tamanho': 'Armas de diferentes tamanhos podem ser usadas por criaturas diferentes.',
        'Veloz': 'Permite fazer um ataque adicional como ação bônus, mas você tem desvantagem no primeiro ataque.',
        'Perfurante': 'Ignora parcialmente a RD de armadura.',
        'Silenciosa': 'Não faz som ao ser usada, útil para emboscadas.',
        'Retorno': 'Após um ataque, a arma retorna para sua mão automaticamente.'
    },

    showPropriedadeInfo(prop) {
        const desc = this.propriedadesArmas[prop] || 'Propriedade não encontrada no banco de dados.';
        this.showToast(desc, 'info');
    },
    ...
    saveArma() {
        ...
        const editIdx = document.getElementById('arma-current-edit').value;
        const categoria = document.getElementById('arma-categoria').value;
        
        let arma;
        if (categoria === 'Arma') {
            const cat = document.getElementById('arma-cat').value;
            arma = {
                nome: document.getElementById('arma-nome').value || "Nova Arma",
                desc: document.getElementById('arma-desc').value || "",
                tipo: document.getElementById('arma-tipo').value,
                cat: cat,
                categoria: 'Arma',
                dano: document.getElementById('arma-dano').value || "1d6",
                attr: document.getElementById('arma-attr').value,
                propriedades: document.getElementById('arma-propriedades').value || "",
                varAtk: parseInt(document.getElementById('arma-var-atk').value) || 0,
                varDano: parseInt(document.getElementById('arma-var-dano').value) || 0
            };
        } else {
            arma = {
                nome: document.getElementById('arma-nome').value || "Novo Item",
                desc: document.getElementById('arma-desc').value || "",
                categoria: categoria,
                cat: "Geral",
                qtd: parseInt(document.getElementById('arma-qtd').value) || 1,
                efeitos: document.getElementById('arma-efeitos').value || ""
            };
        }
        
        if (editIdx !== undefined && editIdx !== "") {
            char.inventario[parseInt(editIdx)] = arma;
        } else {
            char.inventario.push(arma);
        }
        ...
        document.getElementById('arma-desc').value = "";
        document.getElementById('arma-dano').value = "";
        document.getElementById('arma-propriedades').value = "";
        document.getElementById('arma-var-atk').value = "0";
        document.getElementById('arma-var-dano').value = "0";
        document.getElementById('arma-qtd').value = "1";
        document.getElementById('arma-efeitos').value = "";
        document.getElementById('arma-current-edit').value = "";
        document.getElementById('arma-categoria').value = "Arma";
    ...
    renderInventario() {
        ...
        char.inventario.forEach((item, i) => {
            const cat = item.categoria || (item.cat === 'Geral' ? 'Acessório' : 'Arma');
            if (!categorias[cat]) categorias[cat] = [];
            categorias[cat].push({ ...item, originalIdx: i });
        });
        
        const catOrdem = ['Arma', 'Uniforme', 'Escudo', 'Acessório', 'Consumível'];
        let html = '';
        
        catOrdem.forEach(cat => {
            if (categorias[cat] && categorias[cat].length > 0) {
                html += `<div class="section-title" style="margin-top: 15px;"><span>${cat}s</span> <span style="font-size:0.7rem; color:#888;">(${categorias[cat].length})</span></div>`;
                
                html += categorias[cat].map(item => {
                    const isArma = cat === 'Arma';
                    if (isArma) {
                        const props = item.propriedades ? item.propriedades.split(',').map(p => p.trim()) : [];
                        const propsHtml = props.length > 0 ? props.map(p => 
                            `<span onclick="app.showPropriedadeInfo('${p}')" style="cursor:pointer; color:var(--accent); border:1px solid var(--accent); padding:1px 5px; border-radius:3px; font-size:0.65rem; margin-right:3px; display:inline-block;" title="Clique para info">${p}</span>`
                        ).join(' ') : '';
                            ...
                            <div class="list-item-box">
                                <div class="list-item-title" style="align-items: center;">
                                    ...
                                    <div style="display:flex; align-items:center; gap: 10px;">
                                        <button class="btn-roll" onclick="app.rollArma(${item.originalIdx})">Rolar</button>
                                        <button class="btn-edit" onclick="app.editarArma(${item.originalIdx})" style="background:var(--accent-light); color:white; border:none; padding:6px 10px; border-radius:4px; font-size:0.7rem; cursor:pointer;">✏️</button>
                                        <button class="delete-btn" onclick="app.removeArma(${item.originalIdx})">✕</button>
                                    </div>
                                </div>
                                <div class="list-item-desc" style="margin-top: 8px;">
                                    ${item.desc ? `<i>"${item.desc}"</i><br>` : ''}
                                    <strong>Dano:</strong> ${item.dano} | <strong>Attr:</strong> ${item.cat === 'Corpo a corpo' ? item.attr : 'N/A'} <br>
                                    ${propsHtml ? `<div style="margin-top:5px;">${propsHtml}</div>` : ''}
                                    <strong style="color:var(--accent)">Variáveis:</strong> Atk ${item.varAtk >= 0 ? '+' + item.varAtk : item.varAtk} / Dano ${item.varDano >= 0 ? '+' + item.varDano : item.varDano}
                                </div>
                            </div>
                        `;
                    } else {
                        return `
                            <div class="list-item-box">
                                <div class="list-item-title" style="align-items: center;">
                                    <span>${item.nome} <small style="color:var(--accent); font-weight:normal; font-size:0.7rem; display:block;">${cat} (x${item.qtd || 1})</small></span>
                                    <div style="display:flex; align-items:center; gap: 10px;">
                                        <button class="btn-edit" onclick="app.editarArma(${item.originalIdx})" style="background:var(--accent-light); color:white; border:none; padding:6px 10px; border-radius:4px; font-size:0.7rem; cursor:pointer;">✏️</button>
                                        <button class="delete-btn" onclick="app.removeArma(${item.originalIdx})">✕</button>
                                    </div>
    ...
    rollArma(idx) {
        ...
        const finalDanoMod = baseDanoMod + arma.varDano + bonusDano + metNivel + treino;
        ...
        this.enviarRolagemCampanha(arma.nome + ' (Dano)', resultDano, faces, finalDanoMod);
    ...
    renderLists() {
            ...
            `<div class="list-item-box">
                <div class="list-item-title" style="display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <div style="display:flex; gap:5px;">
                        <button class="btn-edit" onclick="app.editarHabilidade(${i})" style="background:var(--accent-light); color:white; border:none; padding:6px 10px; border-radius:4px; font-size:0.7rem; cursor:pointer;">✏️</button>
                        <button class="delete-btn" onclick="app.removeEntry('hab', ${i})">✕</button>
                    </div>
                </div>
                <div class="list-item-desc">${h.split('|')[1]}</div>
            </div>`
        ).join('');
        ...
        document.getElementById('feiticos-list').innerHTML = char.feiticos.map((f, i) => {
            ...
            const nome = parts[0];
            const desc = parts[1] || '';
            const nivel = parseInt(parts[2]) || 1;
            const dano = parts[3] || '';
            const hasDano = dano && dano.length > 0;
            const tipo = parts[5] || 'ataque';
            
            let buttons = '';
            const feiticoEncoded = btoa(encodeURIComponent(f));
            if (hasDano) {
                if (tipo === 'tr') {
                    buttons = `<div style="display:flex; gap:8px; margin:5px 0;">
                        <button class="btn-roll" style="background:var(--accent);" onclick="app.rollFeiticoById('${feiticoEncoded}', 'tr')">TR ${dano}</button>
                    </div>`;
                } else {
                    buttons = `<div style="display:flex; gap:8px; margin:5px 0;">
                        <button class="btn-roll" onclick="app.rollFeiticoById('${feiticoEncoded}', 'ataque')">Atk ${dano}</button>
                    </div>`;
                }
            }
            ...
            return `<div class="list-item-box">
                <div class="list-item-title" style="display:flex; justify-content:space-between; align-items:center;">
                    <span>${nome} <span style="color:var(--hp); font-size:0.7rem;">[Nív ${nivel}]</span></span>
                    <div style="display:flex; gap:5px;">
                        <button class="btn-edit" onclick="app.editarFeitico(${i})" style="background:var(--accent-light); color:white; border:none; padding:6px 10px; border-radius:4px; font-size:0.7rem; cursor:pointer;">✏️</button>
    ...
    resetHabForm() {
        ...
        document.getElementById('hab-edit-idx').value = '';
    },
    ...
    resetFeiticoForm() {
        ...
        document.getElementById('feitico-edit-idx').value = '';
    },
    ...
    addCustomHabilidade() {
        ...
        if (!nome) {
            return this.showToast('Digite o nome da habilidade', 'error');
        }
        
        const char = this.characters[this.activeIdx];
        if (!char.habilidades) char.habilidades = [];
        
        const editIdx = document.getElementById('hab-edit-idx').value;
        
        if (editIdx !== undefined && editIdx !== "") {
            char.habilidades[parseInt(editIdx)] = nome + '|' + desc;
        } else {
        ...
        this.showToast(editIdx !== undefined && editIdx !== '' ? 'Habilidade atualizada!' : 'Habilidade criada!', 'success');
    },

    addCustomFeitico() {
        const nome = document.getElementById('feitico-nome').value.trim();
        const desc = document.getElementById('feitico-desc').value.trim();
        const nivel = parseInt(document.getElementById('feitico-nivel').value) || 1;
        const dano = document.getElementById('feitico-dano').value.trim();
        const tipo = document.getElementById('feitico-tipo').value;
        
        if (!nome) {
            return this.showToast('Digite o nome do feitiço', 'error');
        }
        
        const char = this.characters[this.activeIdx];
        if (!char.feiticos) char.feiticos = [];
        
        const editIdx = document.getElementById('feitico-edit-idx').value;
        const danoAttr = dano.match(/\+([A-Z]{2,3})/i)?.[1] || 'INT';
        const feiticoData = nome + '|' + desc + '|' + nivel + '|' + dano + '|' + danoAttr + '|' + tipo;
        
        if (editIdx !== undefined && editIdx !== "") {
            char.feiticos[parseInt(editIdx)] = feiticoData;
        } else {
        ...
        document.getElementById('feitico-nome').value = '';
        document.getElementById('feitico-desc').value = '';
        document.getElementById('feitico-nivel').value = '1';
        document.getElementById('feitico-dano').value = '';
        document.getElementById('feitico-tipo').value = 'ataque';
        document.getElementById('feitico-edit-idx').value = '';
        
        this.renderLists();
        this.saveData();
        ui.closeModal('modal-feitico');
        this.showToast(editIdx !== undefined && editIdx !== '' ? 'Feitiço atualizado!' : 'Feitiço criado!', 'success');
    },

    editarFeitico(idx) {
        const char = this.characters[this.activeIdx];
        const parts = char.feiticos[idx].split('|');
        
        document.getElementById('feitico-nome').value = parts[0] || '';
        document.getElementById('feitico-desc').value = parts[1] || '';
        document.getElementById('feitico-nivel').value = parseInt(parts[2]) || 1;
        document.getElementById('feitico-dano').value = parts[3] || '';
        document.getElementById('feitico-tipo').value = parts[5] || 'ataque';
        document.getElementById('feitico-edit-idx').value = idx;
        
        ui.openModal('modal-feitico');
    },

editarHabilidade(idx) {
        const char = this.characters[this.activeIdx];
        const parts = char.habilidades[idx].split('|');
        
        document.getElementById('hab-nome').value = parts[0] || '';
        document.getElementById('hab-desc').value = parts[1] || '';
        document.getElementById('hab-edit-idx').value = idx;
        
        ui.openModal('modal-hab');
    },

    rollFeiticoById(encoded, tipo) {
        try {
            const feat = decodeURIComponent(atob(encoded));
            this.rollFeitico(feat, tipo);
        } catch(e) {
            console.error('Erro ao decodificar feitiço:', e);
            this.showToast('Erro ao carregar feitiço', 'error');
        }
    },

    rollFeitico(feat, tipo = 'ataque') {
        try {
            const parts = feat.split('|');
            const nome = parts[0] || 'Feitiço';
            const desc = parts[1] || '';
            const dano = parts[2] || '';
            const attrName = (parts[3] || 'INT').toUpperCase();
            const testeTipo = parts[4] || 'ataque';
            
            if (!dano) {
                this.showToast('Feitiço sem dano definido', 'error');
                return;
            }
            ...
            if (!container) {
                ...
                container.style.cssText = 'position:fixed;bottom:20px;left:50%;transform:translateX(-50%);z-index:10000;display:flex;flex-direction:column-reverse;gap:10px;pointer-events:none;max-width:300px;';
            ...
            const attrMods = {
                FOR: Math.floor((char.attrs.FOR - 10) / 2),
                DEX: Math.floor((char.attrs.DEX - 10) / 2),
                CON: Math.floor((char.attrs.CON - 10) / 2),
                INT: Math.floor((char.attrs.INT - 10) / 2),
                SAB: Math.floor((char.attrs.SAB - 10) / 2),
                PRE: Math.floor((char.attrs.PRE - 10) / 2)
            };
            
            const attrVal = char.attrs[attrName] || 10;
            const mod = isEspTecnica ? Math.floor((attrVal - 10) / 2) : 0;
            
            const result = this.parseDano(dano, isEspTecnica ? attrMods : {});
            
            if (!result || result.total === undefined || isNaN(result.total)) {
                this.showToast('Erro ao calcular: ' + dano, 'error');
                return;
            }
            ...
            // TR (Teste de Resistência) - apenas mostra o dano
            if (testeTipo === 'tr' || testeTipo === 'cd') {
                toast.style.borderColor = 'var(--accent)';
                toast.innerHTML = `
                    <div class="toast-title">${nome} (TR)</div>
                    <div class="toast-value" style="color:var(--accent); font-size:2rem;">${result.total}</div>
                    <div class="toast-math" style="font-size:0.9rem;">Dano: ${dano} (${result.rolls.join(', ')})</div>
                    <div class="toast-math" style="font-size:0.7rem; color:#888;">Alvo faz TR para reduzir dano</div>
                `;
                container.appendChild(toast);
                
                this.enviarRolagemCampanha(nome + ' (TR)', result.total, result.faces || 0, 0);
                
                setTimeout(() => toast.remove(), 5000);
                return;
            }
            
            // Teste de Ataque - mostra Ataque + Dano na mesma caixa
            const d20 = Math.floor(Math.random() * 20) + 1;
            const total = d20 + result.total + mod;
            const signMod = result.total + mod;
            
            // Determinar resultado
            let resultadoTexto = 'ERROU';
            let resultadoCor = 'var(--hp)';
            if (d20 === 20) {
                resultadoTexto = 'ACERTO CRÍTICO!';
                resultadoCor = '#22c55e';
            } else if (d20 === 1) {
                resultadoTexto = 'ERRO CRÍTICO!';
                resultadoCor = 'var(--hp)';
            } else if (total >= 10) {
                resultadoTexto = 'ACERTO';
                resultadoCor = 'var(--accent)';
            }
            
            toast.style.borderColor = d20 === 20 ? 'var(--accent)' : (d20 === 1 ? 'var(--hp)' : 'var(--border)');
            toast.innerHTML = `
                <div class="toast-title">${nome}</div>
                <div class="toast-value" style="color:${resultadoCor}; font-size:1.8rem;">${resultadoTexto}</div>
                <div class="toast-math">Ataque: D20(${d20}) + ${signMod} = ${total}</div>
                <div class="toast-math">Dano: ${dano} (${result.rolls.join(', ')}) = ${result.total}</div>
            `;
            
            container.appendChild(toast);
            
            // Enviar para campaña
            this.enviarRolagemCampanha(nome, total, 20, signMod);
            
            setTimeout(() => toast.remove(), 5000);
        } catch(e) {
            console.error('Erro em rollFeitico:', e);
        }
    },
    ...
    addCustomTalento() {
        ...
        if (!nome) {
            return this.showToast('Digite o nome do talento', 'error');
        }
        ...
        this.showToast('Talento criado!', 'success');
    },
    ...
    renderAptPerfil() {
        const char = this.characters[this.activeIdx];
        const aptList = ['Aura', 'Controle e Leitura', 'Barreira', 'Domínio', 'Energia Reversa'];
        
        if (!char.perfilAmaldicado) {
            char.perfilAmaldicado = {
                aptidoes: aptList.map(nome => ({ nome, nivel: 0, tecnicas: [] })),
                tecnicas: [],
                expansao: { nome: '', tipo: '', desc: '' },
                tecnicaMax: { nome: '', desc: '' }
            };
        ...
        let aptidoesMap = {};
        char.perfilAmaldicado.aptidoes.forEach(apt => {
            aptidoesMap[apt.nome] = apt;
        });
        
        const html = aptList.map((nome, idx) => {
            const apt = aptidoesMap[nome] || { nome, nivel: 0, tecnicas: [] };
            const nivel = apt.nivel || 0;
            const tecnicas = apt.tecnicas || [];
            
            return `
                <div style="background:var(--input); padding:12px; border-radius:6px; margin-bottom:12px; border:1px solid var(--border);">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 10px;">
                        <span style="color:var(--accent); font-weight:bold; font-size:0.95rem;">${nome}</span>
                        <div style="display:flex; align-items:center; gap: 8px;">
                            <label style="font-size: 0.75rem; color: #888;">Nível:</label>
                            <input type="number" value="${nivel}" min="0" max="10" 
                                style="width: 55px; background:var(--bg); border:1px solid var(--border); color:var(--text); padding: 6px; border-radius:4px; text-align:center; font-weight:bold;"
                                onchange="app.updateAptNivel('${nome}', this.value)">
                        </div>
                    </div>
                    
                    <div style="border-top:1px solid var(--border); padding-top:10px;">
                        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                            <span style="font-size:0.75rem; color:#888; font-weight:bold;">TÉCNICAS</span>
                            <button class="btn-roll" style="padding:4px 8px; font-size:0.7rem;" onclick="app.openAddTecnicaApt('${nome}')">+ Adicionar</button>
                        </div>
                        <div id="tecnicas-apt-${idx}">
                            ${tecnicas.map((tec, i) => `
                                <div style="background:var(--bg); padding:8px; border-radius:4px; margin-bottom:6px; border:1px solid var(--border);">
                                    <div style="display:flex; justify-content:space-between; align-items:flex-start;">
                                        <div>
                                            <span style="color:var(--text); font-weight:bold; font-size:0.85rem;">${tec.nome}</span>
                                            <span style="font-size:0.65rem; color:var(--accent); margin-left:6px;">Nv.${tec.nivel}</span>
                                        </div>
                                        <button class="delete-btn" style="padding:2px 6px; font-size:0.7rem;" onclick="app.removeTecnicaApt('${nome}', ${i})">✕</button>
                                    </div>
                                    <p style="font-size:0.75rem; color:#888; margin-top:4px; margin-bottom:0;">${tec.desc || ''}</p>
                                </div>
                            `).join('') || '<p style="color:#666; font-size:0.75rem; text-align:center; padding:8px;">Nenhuma técnica</p>'}
                        </div>
                    </div>
                </div>
            `;
        ...
        document.getElementById('apt-perfil-all').innerHTML = html;
        this.renderTecnicasPorApt();
    },

    updateAptNivel(nome, nivel) {
        const char = this.characters[this.activeIdx];
        if (!char.perfilAmaldicado) return;
        
        let apt = char.perfilAmaldicado.aptidoes.find(a => a.nome === nome);
        if (apt) {
            apt.nivel = parseInt(nivel) || 0;
        } else {
            char.perfilAmaldicado.aptidoes.push({ nome, nivel: parseInt(nivel) || 0, tecnicas: [] });
        }
        this.saveData();
    },

    openAddTecnicaApt(aptNome) {
        document.getElementById('tecnica-apt-nome').value = '';
        document.getElementById('tecnica-apt-nivel').value = '0';
        document.getElementById('tecnica-apt-desc').value = '';
        document.getElementById('tecnica-apt-target').value = aptNome;
        ui.openModal('modal-tecnica-apt');
    },
    ...
    addTecnicaAptFromModal() {
        ...
        const aptNome = document.getElementById('tecnica-apt-target').value;
        ...
        let apt = char.perfilAmaldicado.aptidoes.find(a => a.nome === aptNome);
        if (!apt) {
            apt = { nome: aptNome, nivel: 0, tecnicas: [] };
            char.perfilAmaldicado.aptidoes.push(apt);
        }
        
        if (!apt.tecnicas) apt.tecnicas = [];
        apt.tecnicas.push({ nome, nivel: parseInt(nivel), desc });
    ...
    removeTecnicaApt(aptNome, tecIdx) {
        const char = this.characters[this.activeIdx];
        if (!char.perfilAmaldicado) return;
        
        const apt = char.perfilAmaldicado.aptidoes.find(a => a.nome === aptNome);
        if (apt && apt.tecnicas) {
            apt.tecnicas.splice(tecIdx, 1);
            this.renderAptPerfil();
            this.saveData();
        }
    },
    ...
    savePerfilCDs() {
        ...
        char.perfilAmaldicado.cdTecnica = parseInt(document.getElementById('cd-tecnica-perfil').value) || 10;
        char.perfilAmaldicado.cdAmaldicada = parseInt(document.getElementById('cd-amaldicada-perfil').value) || 10;
        char.perfilAmaldicado.cdEspecializacao = parseInt(document.getElementById('cd-especializacao-perfil').value) || 10;
        this.saveData();
    },
    ...
    saveVoto() {
        const char = this.characters[this.activeIdx];
        if (!char.perfilAmaldicado) {
            char.perfilAmaldicado = { aptidoes: [], tecnicas: [], expansao: { nome: '', tipo: '', desc: '' }, tecnicaMax: { nome: '', desc: '' }, votos: [] };
        }
        if (!char.perfilAmaldicado.votos) char.perfilAmaldicado.votos = [];
        
        const treino = this.getTreinoBonus(char.lvl);
        if (char.perfilAmaldicado.votos.length >= treino) {
            return this.showToast(`Máximo de ${treino} votos permitidos (Bônus de Treino)!`, 'error');
        }
        
        const categoria = document.getElementById('voto-categoria').value;
        const tipo = document.getElementById('voto-tipo').value;
        const peso = document.getElementById('voto-peso').value;
        const nome = document.getElementById('voto-nome').value.trim();
        const beneficio = document.getElementById('voto-desc').value.trim();
        const maleficio = document.getElementById('voto-maleficio').value.trim();
        
        if (!nome) {
            return this.showToast('Digite o nome do voto!', 'error');
        }
        
        if (peso === 'Extremo' && tipo === 'Temporario') {
            return this.showToast('Votos Temporários não podem ser Extremos!', 'error');
        }
        
        char.perfilAmaldicado.votos.push({ categoria, tipo, peso, nome, beneficio, maleficio });
        
        document.getElementById('voto-categoria').value = 'Proprio';
        document.getElementById('voto-tipo').value = 'Temporario';
        document.getElementById('voto-peso').value = 'Leve';
        document.getElementById('voto-nome').value = '';
        document.getElementById('voto-desc').value = '';
        document.getElementById('voto-maleficio').value = '';
        
        this.saveData();
        this.renderVotos();
        this.showToast('Voto salvo!');
    },

    removeVoto(idx) {
        const char = this.characters[this.activeIdx];
        if (char.perfilAmaldicado.votos) {
            char.perfilAmaldicado.votos.splice(idx, 1);
            this.saveData();
            this.renderVotos();
        }
    },
    ...
    renderVotos() {
        const char = this.characters[this.activeIdx];
        const container = document.getElementById('votos-list');
        const countSpan = document.getElementById('votos-count');
        if (!container) return;
        
        const treino = this.getTreinoBonus(char.lvl || 1);
        
        if (countSpan) {
            countSpan.innerText = `( ${(char.perfilAmaldicado?.votos?.length || 0)} / ${treino} )`;
        }
        
        if (!char.perfilAmaldicado?.votos || char.perfilAmaldicado.votos.length === 0) {
            container.innerHTML = '<p style="text-align:center; color:#888; font-size:0.8rem;">Nenhum voto adicionado.</p>';
            return;
        }
        
        const pesoCores = { 'Leve': '#22c55e', 'Medio': '#eab308', 'Pesado': '#f97316', 'Extremo': '#ef4444' };
        const catIcon = { 'Proprio': '👤', 'Emergencial': '⚡', 'Contratual': '🤝' };
        
        container.innerHTML = char.perfilAmaldicado.votos.map((v, i) => `
            <div style="background:var(--input); padding:10px; border-radius:6px; margin-bottom:8px; border-left: 3px solid ${pesoCores[v.peso] || '#888'};">
                <div style="display:flex; justify-content:space-between; align-items:center;">
                    <span style="color:var(--accent); font-weight:bold; font-size:0.9rem;">${catIcon[v.categoria] || ''} ${v.nome}</span>
                    <button class="delete-btn" onclick="app.removeVoto(${i})">✕</button>
                </div>
                <div style="display:flex; gap:8px; margin:5px 0;">
                    ${v.tipo ? `<span style="background:var(--border); color:var(--text); padding:2px 6px; border-radius:3px; font-size:0.65rem;">${v.tipo}</span>` : ''}
                    ${v.peso ? `<span style="background:${pesoCores[v.peso]}; color:white; padding:2px 6px; border-radius:3px; font-size:0.65rem;">${v.peso}</span>` : ''}
                </div>
                ${v.beneficio ? `<p style="color:#22c55e; font-size:0.75rem; margin:5px 0;"><strong>✓ Benefício:</strong> ${v.beneficio}</p>` : ''}
                ${v.maleficio ? `<p style="color:#ef4444; font-size:0.75rem; margin:5px 0;"><strong>✗ Malefício:</strong> ${v.maleficio}</p>` : ''}
            </div>
        `).join('');
    },
    ...
    saveInvocacao() {
        ...
        const editIdx = document.getElementById('invoca-edit-idx').value;
        
        if (!editIdx && char.invocacoes.length >= 15) {
        ...
        const acoes = [
            document.getElementById('invoca-acao1').value,
            document.getElementById('invoca-acao2').value,
            document.getElementById('invoca-acao3').value,
            document.getElementById('invoca-acao4').value,
            document.getElementById('invoca-acao5').value
        ].filter(a => a.trim());
        
        const carac = [
            document.getElementById('invoca-carac1').value,
            document.getElementById('invoca-carac2').value,
            document.getElementById('invoca-carac3').value,
            document.getElementById('invoca-carac4').value,
            document.getElementById('invoca-carac5').value
        ].filter(c => c.trim());
        
        const invoca = {
            nome: document.getElementById('invoca-nome').value || 'Invocação',
            desc: document.getElementById('invoca-desc').value || '',
            tipo: document.getElementById('invoca-tipo').value || 'shikigami',
            grau: parseInt(document.getElementById('invoca-grau').value) || 4,
            dano: document.getElementById('invoca-dano').value || '',
            cd: parseInt(document.getElementById('invoca-cd').value) || 10,
            atkTreinado: document.getElementById('invoca-atk-treinado').value || '',
            trTreinado: document.getElementById('invoca-tr-treinado').value || '',
            acoes: acoes,
            carac: carac,
            attrs: {
                FOR: parseInt(document.getElementById('invoca-for').value) || 8,
                DEX: parseInt(document.getElementById('invoca-dex').value) || 8,
                CON: parseInt(document.getElementById('invoca-con').value) || 8,
                INT: parseInt(document.getElementById('invoca-int').value) || 8,
                SAB: parseInt(document.getElementById('invoca-sab').value) || 8,
                PRE: parseInt(document.getElementById('invoca-pre').value) || 8
            },
        ...
        if (editIdx !== undefined && editIdx !== "") {
            char.invocacoes[parseInt(editIdx)] = invoca;
        } else {
        ...
        document.getElementById('invoca-edit-idx').value = '';
        ...
        this.showToast(editIdx !== undefined && editIdx !== '' ? 'Invocação atualizada!' : 'Invocação criada!', 'success');
    },

    calcularInvocacaoStats() {
        const char = this.characters[this.activeIdx];
        const grau = parseInt(document.getElementById('invoca-grau').value) || 4;
        const lvl = char.lvl || 1;
        const treino = this.getTreinoBonus(lvl);
        
        const con = parseInt(document.getElementById('invoca-con').value) || 10;
        const dex = parseInt(document.getElementById('invoca-dex').value) || 10;
        ...
        const conHalf = (con - 10) / 2;
        
        switch(grau) {
            case 4:
                pv = 10 + Math.floor(conHalf) + lvl;
                defesa = 10 + dexMod + treino;
                break;
            case 3:
                pv = 25 + Math.floor(conHalf) + lvl;
                defesa = 12 + dexMod + treino;
                break;
            case 2:
                pv = 40 + (con - 10) + lvl;
                defesa = 16 + dexMod + treino;
                break;
            case 1:
                pv = 60 + (con - 10) + Math.floor(lvl * 1.5);
                defesa = 20 + dexMod + treino;
                break;
            case 0:
                pv = 80 + (con - 10) + (lvl * 2);
                defesa = 24 + dexMod + treino;
                break;
            default:
                pv = 10 + Math.floor(conHalf) + lvl;
                defesa = 10 + dexMod + treino;
        ...
        document.getElementById('invoca-pv').value = Math.max(1, pv);
        document.getElementById('invoca-def').value = Math.max(1, defesa);
    },

    

    renderInvocaSkillsInModal() {
        const container = document.getElementById('invoca-skill-list');
        if (!container) return;
        
        const grau = parseInt(document.getElementById('invoca-grau').value) || 4;
        const intVal = parseInt(document.getElementById('invoca-int').value) || 8;
        const sabVal = parseInt(document.getElementById('invoca-sab').value) || 8;
        const modInt = Math.floor((intVal - 10) / 2);
        const modSAB = Math.floor((sabVal - 10) / 2);
        const modMagico = Math.max(modInt, modSAB);
        
        let basePericias = 1 + Math.floor(modMagico / 2);
        if (basePericias < 1) basePericias = 1;
        
        let periciasGrau = 1;
        if (grau <= 3) periciasGrau = 1;
        else if (grau <= 1) periciasGrau = 2;
        else if (grau === 0) periciasGrau = 3;
        
        const totalPericias = basePericias + periciasGrau;
        
        const limitSpan = document.getElementById('invoca-pericias-limit');
        if (limitSpan) {
            limitSpan.innerText = `Máx: ${totalPericias} (${basePericias} base + ${periciasGrau} pelo grau)`;
        }
        
        container.innerHTML = '';
        
        this.skillsRef.forEach(s => {
            if (s.n === 'Ofício') return;
            ...
            div.innerHTML = `
                <label style="display:flex; align-items:center; gap:5px; font-size:0.7rem;">
                    <input type="checkbox" class="custom-checkbox invoca-pericia-checkbox" value="${s.n}" data-pericia="${s.n}" onchange="app.checkInvocaPericiaLimit(${totalPericias})">
    ...
    checkInvocaPericiaLimit(maxPericias) {
        const checkboxes = document.querySelectorAll('#invoca-skill-list .invoca-pericia-checkbox:checked');
        if (checkboxes.length > maxPericias) {
            this.showToast(`Máximo de ${maxPericias} perícias permitidas!`, 'error');
...
updateCondicoes() {
        const condicoes = {
            abalado: document.getElementById('cond-abalado').checked,
            exposto: document.getElementById('cond-exposto').checked,
            amedrontado: document.getElementById('cond-amdc').checked,
            enredado: document.getElementById('cond-enredado').checked,
            lento: document.getElementById('cond-lento').checked,
            ciego: document.getElementById('cond-ciego').checked,
            imvel: document.getElementById('cond-imóvel').checked,
            aterrorizado: document.getElementById('cond-aterrorizado').checked,
            agarrado: document.getElementById('cond-agarrado').checked,
            paralizado: document.getElementById('cond-paralizado').checked,
            inconsciente: document.getElementById('cond-inconsciente').checked,
            surdo: document.getElementById('cond-surdo').checked,
            confuse: document.getElementById('cond-confuso').checked,
            envenenado: document.getElementById('cond-envenenado').checked,
            condenado: document.getElementById('cond-condenado').checked,
            enjoado: document.getElementById('cond-enjoado').checked,
            engasgando: document.getElementById('cond-engasgando').checked,
            surpreso: document.getElementById('cond-surpreso').checked
        };
        
        const char = this.characters[this.activeIdx];
        if (!char.condicoes) char.condicoes = {};
        char.condicoes = condicoes;
        
        // Calcular modificadores de condições
        let mods = { atk: 0, pericia: 0, def: 0, tr: 0 };
        
        // Abalado (Fraca): -1 em ataques E testes de perícia
        if (condicoes.abalado) {
            mods.atk += -1;
            mods.pericia += -1;
        }
        
        // Exposto (Forte): +4 em jogadas de ataque CONTRA a criatura + dano adicional
        // Isso afeta a defesa, não ataque próprio
        if (condicoes.exposto) {
            // Não afeta eigene ataque, mas inimigos têm +4
        }
        
        // Amedrontado (Média): -3 em ataques e testes
        if (condicoes.amedrontado) {
            mods.atk += -3;
            mods.pericia += -3;
        }
        
        // Enredado (Média): -2 em ataques E defesa, deslocamento/2
        if (condicoes.enredado) {
            mods.atk += -2;
            mods.def += -2;
        }
        
        // Lento (Média): movimento reduzido pela metade (não afeta combate)
        
        // Cego (Forte): -2 ataques, -5 defesa e -5 percepção
        if (condicoes.ciego) {
            mods.atk += -2;
            mods.def += -5;
            mods.pericia += -5;
        ...
        // Imóvel (Forte): -10 defesa e falha automática Reflexos
        if (condicoes.imvel) {
            mods.def += -10;
            mods.tr += -10;
        }
        
        // Aterrorizado (Forte): -4 em ataques e testes
        if (condicoes.aterrorizado) {
            mods.atk += -4;
            mods.pericia += -4;
        }
        
        // Agarrado (Média): Imóvel (-10 defesa) e falha Reflexos
        if (condicoes.agarrado) {
            mods.def += -10;
            mods.tr += -10;
        }
        
        // Paralizado (Extrema): Imóvel + não pode agir
        if (condicoes.paralizado) {
            mods.def += -10;
            mods.tr += -10;
        }
        
        // Inconsciente (Extrema): Caído + não pode agir, -10 defesa
        if (condicoes.inconsciente) {
            mods.def += -10;
            // Não pode realizar ações
        }
        
        // Surdo (Média): -5 em testes de percepção
        if (condicoes.surdo) {
            mods.pericia += -5;
        }
        
        // Confuso (Média): -4 em Fortitude e Atletismo
        if (condicoes.confuse) {
            mods.tr += -4;
        }
        
        // Envenenado (Média): -2 em ataques, testes de resistência e perícia
        if (condicoes.envenenado) {
            mods.atk += -2;
            mods.pericia += -2;
            mods.tr += -2;
        }
        
        // Condenado (Média): custo de PE +1 (não afeta combate)
        
        // Enjoado (Média): não pode converter ações (não afeta combate)
        
        // Engasgando (Média): mudo (não afeta combate)
        
        // Surpreso: -3 na defesa
        if (condicoes.surpreso) {
            mods.def += -3;
        }
        
        // Armazenar mods no personagem
        char.condicaoMods = mods;
        
        // Atualizar display de efeitos
        let efeitos = [];
        
        if (condicoes.abalado) efeitos.push("-1 Ataque/Testes");
        if (condicoes.exposto) efeitos.push("Inimigos: +4 ATAQUE");
        if (condicoes.amedrontado) efeitos.push("-3 Ataque/Testes");
        if (condicoes.enredado) efeitos.push("-2 Ataque/Defesa");
        if (condicoes.ciego) efeitos.push("-2 ATK, -5 DEF, -5 Percepção");
        if (condicoes.imvel) efeitos.push("-10 DEF, falha Reflexos");
        if (condicoes.aterrorizado) efeitos.push("-4 Ataque/Testes");
        if (condicoes.agarrado) efeitos.push("-10 DEF, Imóvel");
        if (condicoes.paralizado) efeitos.push("-10 DEF, Imóvel");
        if (condicoes.inconsciente) efeitos.push("Caído, -10 DEF");
        if (condicoes.surdo) efeitos.push("-5 Percepção");
        if (condicoes.confuse) efeitos.push("-4 TR Fort/Atl");
        if (condicoes.envenenado) efeitos.push("-2 ATK/Testes/TR");
        if (condicoes.surpreso) efeitos.push("-3 Defesa");
        
        const container = document.getElementById('condicoes-efeitos');
        const texto = document.getElementById('cond-efeitos-texto');
        
        if (efeitos.length > 0) {
            container.style.display = 'block';
            texto.innerText = efeitos.join(' | ');
        } else {
            container.style.display = 'none';
        }
        
        // Mostrar toast com os modificadores ativos
        let modMsg = '';
        if (mods.atk !== 0) modMsg += ` ATK: ${mods.atk > 0 ? '+' : ''}${mods.atk}`;
        if (mods.pericia !== 0) modMsg += ` Perícia: ${mods.pericia > 0 ? '+' : ''}${mods.pericia}`;
        if (mods.def !== 0) modMsg += ` DEF: ${mods.def > 0 ? '+' : ''}${mods.def}`;
        if (mods.tr !== 0) modMsg += ` TR: ${mods.tr > 0 ? '+' : ''}${mods.tr}`;
        
        if (modMsg) {
            this.showToast('📊 Condições ativas:' + modMsg, 'info');
        }
    ...
    updateCondicoesDisplay() {
        const char = this.characters[this.activeIdx];
        if (!char.condicoes) return;
        ...
        // Also show toast with active mods
        const mods = char.condicaoMods || { atk: 0, pericia: 0, def: 0, tr: 0 };
        let modMsg = '';
        if (mods.atk !== 0) modMsg += ` ATK: ${mods.atk > 0 ? '+' : ''}${mods.atk}`;
        if (mods.pericia !== 0) modMsg += ` Perícia: ${mods.pericia > 0 ? '+' : ''}${mods.pericia}`;
        if (mods.def !== 0) modMsg += ` DEF: ${mods.def > 0 ? '+' : ''}${mods.def}`;
    ...
    renderInvocacoes() {
        ...
        if (!char.invocacoes || char.invocacoes.length === 0) {
            container.innerHTML = '<p style="text-align:center; color:#888; margin:30px 0;">Nenhuma invocação criada.</p>';
            return;
        }
        
        const grauNome = ['', '1º', '2º', '3º', '4º', 'Esp.'];
        
        container.innerHTML = char.invocacoes.map((inv, i) => {
            const grau = grauNome[inv.grau] || '4º';
            const tipoNome = inv.tipo === 'corpo' ? 'Corpo Amaldiçoado' : 'Shikigami';
            const tipoCor = inv.tipo === 'corpo' ? '#ef4444' : '#9d4edd';
            return `<div class="invoca-card" style="background:var(--card); border:1px solid var(--border); border-radius:8px; padding:15px; margin-bottom:10px; position:relative; animation:slideIn 0.3s ease;">
                <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:10px;">
                    <div>
                        <span style="color:var(--accent); font-weight:bold; font-size:1rem;">${inv.nome}</span>
                        <span style="background:${tipoCor}; color:white; padding:2px 6px; border-radius:3px; font-size:0.6rem; margin-left:5px;">${tipoNome}</span>
                        <span style="color:var(--hp); font-size:0.75rem; margin-left:8px;">${grau}</span>
                    </div>
                    <div style="display:flex; gap:5px;">
                        <button class="btn-edit" onclick="app.editInvocacao(${i})" style="background:var(--accent); color:white; border:none; padding:5px 10px; border-radius:4px; font-size:0.75rem; cursor:pointer;">✏️</button>
                        <button class="delete-btn" onclick="app.removeInvocacao(${i})">✕</button>
                    </div>
                </div>
                ${inv.desc ? `<p style="color:#888; font-size:0.8rem; margin:5px 0;">${inv.desc}</p>` : ''}
                <div style="display:flex; gap:15px; flex-wrap:wrap; margin-top:10px;">
                    <div style="background:var(--input); padding:5px 10px; border-radius:4px; font-size:0.75rem; display:flex; align-items:center; gap:5px;">
                        <span style="color:#888;">PV:</span>
                        <button onclick="app.adjustInvocaPV(${i}, -1)" style="background:var(--hp); color:white; border:none; padding:2px 6px; border-radius:3px; cursor:pointer;">-</button>
                        <span style="color:var(--hp); font-weight:bold;">${inv.pv}</span>
                        <button onclick="app.adjustInvocaPV(${i}, 1)" style="background:var(--hp); color:white; border:none; padding:2px 6px; border-radius:3px; cursor:pointer;">+</button>
                        <span style="color:#666;">/ ${inv.pvMax}</span>
                    </div>
                    <div style="background:var(--input); padding:5px 10px; border-radius:4px; font-size:0.75rem;">
                        <span style="color:#888;">DEF:</span> <span style="color:var(--energy); font-weight:bold;">${inv.defesa}</span>
                    </div>
                    <div style="background:var(--input); padding:5px 10px; border-radius:4px; font-size:0.75rem;">
                        <span style="color:#888;">CD:</span> <span style="color:var(--accent); font-weight:bold;">${inv.cd}</span>
                    </div>
                    ${inv.atkTreinado ? `<div style="background:var(--input); padding:5px 10px; border-radius:4px; font-size:0.75rem;">
                        <span style="color:#888;">ATK:</span> <span style="color:var(--hp); font-weight:bold;">${inv.atkTreinado}</span>
                    </div>` : ''}
                    ${inv.trTreinado ? `<div style="background:var(--input); padding:5px 10px; border-radius:4px; font-size:0.75rem;">
                        <span style="color:#888;">TR:</span> <span style="color:#8b5cf6; font-weight:bold;">${inv.trTreinado}</span>
                    </div>` : ''}
                    ${inv.dano ? `<div style="background:var(--input); padding:5px 10px; border-radius:4px; font-size:0.75rem;">
                        <span style="color:#888;">Dano:</span> <span style="color:var(--hp); font-weight:bold;">${inv.dano}</span>
                    </div>` : ''}
                </div>
                <div style="margin-top:10px; display:flex; gap:8px;">
                    <button class="btn-roll" onclick="app.rollInvocaAtk(${i}, 'FOR')">⚔️ ATK</button>
                    ${inv.dano ? `<button class="btn-roll" style="background:var(--hp);" onclick="app.rollInvocaDano(${i})">💥 Dano</button>` : ''}
                </div>
            </div>`;
        }).join('');
    },
    ...
    editInvocacao(idx) {
        const inv = this.characters[this.activeIdx].invocacoes[idx];
        
        document.getElementById('invoca-edit-idx').value = idx;
        document.getElementById('invoca-tipo').value = inv.tipo || 'shikigami';
        document.getElementById('invoca-grau').value = inv.grau || 4;
        document.getElementById('invoca-nome').value = inv.nome || '';
        document.getElementById('invoca-desc').value = inv.desc || '';
        document.getElementById('invoca-pv').value = inv.pv || 10;
        document.getElementById('invoca-def').value = inv.defesa || 10;
        document.getElementById('invoca-cd').value = inv.cd || 10;
        document.getElementById('invoca-desloc').value = inv.desloc || '9m';
        document.getElementById('invoca-dano').value = inv.dano || '';
        document.getElementById('invoca-atk-treinado').value = inv.atkTreinado || '';
        document.getElementById('invoca-tr-treinado').value = inv.trTreinado || '';

        document.getElementById('invoca-acao1').value = (inv.acoes && inv.acoes[0]) || '';
        document.getElementById('invoca-acao2').value = (inv.acoes && inv.acoes[1]) || '';
        document.getElementById('invoca-acao3').value = (inv.acoes && inv.acoes[2]) || '';
        document.getElementById('invoca-acao4').value = (inv.acoes && inv.acoes[3]) || '';
        document.getElementById('invoca-acao5').value = (inv.acoes && inv.acoes[4]) || '';
        document.getElementById('invoca-carac1').value = (inv.carac && inv.carac[0]) || '';
        document.getElementById('invoca-carac2').value = (inv.carac && inv.carac[1]) || '';
        document.getElementById('invoca-carac3').value = (inv.carac && inv.carac[2]) || '';
        document.getElementById('invoca-carac4').value = (inv.carac && inv.carac[3]) || '';
        document.getElementById('invoca-carac5').value = (inv.carac && inv.carac[4]) || '';
        
        if (inv.attrs) {
            document.getElementById('invoca-for').value = inv.attrs.FOR || 8;
            document.getElementById('invoca-dex').value = inv.attrs.DEX || 8;
            document.getElementById('invoca-con').value = inv.attrs.CON || 8;
            document.getElementById('invoca-int').value = inv.attrs.INT || 8;
            document.getElementById('invoca-sab').value = inv.attrs.SAB || 8;
            document.getElementById('invoca-pre').value = inv.attrs.PRE || 8;
        }
        
        this.renderInvocaSkillsInModal();
        
        if (inv.pericias && inv.pericias.length > 0) {
            setTimeout(() => {
                inv.pericias.forEach(p => {
                    const cb = document.querySelector(`#invoca-skill-list .invoca-pericia-checkbox[value="${p}"]`);
    ...
    renderShikigamisResumo() {
        const char = this.characters[this.activeIdx];
        const container = document.getElementById('shikigamis-resumo-list');
        
        if (!char.invocacoes || char.invocacoes.length === 0) {
            container.innerHTML = '<p style="text-align:center; color:#888; font-size:0.8rem; margin:10px 0;">Nenhum Shikigami invocado</p>';
            return;
        }
        
        // Filtrar apenas shikigamis com PV > 0 (invocados)
        const invocados = char.invocacoes.filter(inv => inv.pv > 0);
        
        if (invocados.length === 0) {
            container.innerHTML = '<p style="text-align:center; color:#888; font-size:0.8rem; margin:10px 0;">Nenhum Shikigami invocado</p>';
            return;
        }
        ...
        container.innerHTML = invocados.map((inv) => {
            // Encontrar o índice original no array de invocacoes
            const originalIdx = char.invocacoes.indexOf(inv);
            const grau = grauNome[inv.grau] || '4º';
            return `<div style="display:flex; justify-content:space-between; align-items:center; padding:8px; background:var(--input); border-radius:4px; margin-bottom:5px; border:1px solid var(--accent);">
                <div style="flex:1;">
                    <span style="color:var(--accent); font-weight:bold; font-size:0.9rem;">${inv.nome}</span>
                    <span style="color:var(--hp); font-size:0.75rem; margin-left:5px;">${grau}</span>
                </div>
                <div style="display:flex; gap:10px; align-items:center;">
                    <span style="background:var(--hp); color:white; padding:2px 6px; border-radius:3px; font-size:0.7rem;">PV ${inv.pv}/${inv.pvMax}</span>
                    <button class="btn-roll" style="padding:4px 8px; font-size:0.7rem;" onclick="app.rollInvocaAtk(${originalIdx}, 'FOR')">ATK</button>
                    <button class="btn-roll" style="background:var(--hp); padding:4px 8px; font-size:0.7rem;" onclick="app.rollInvocaDano(${originalIdx})">Dano</button>
                </div>
            </div>`;
        }).join('');
    },
    ...
    condicoesRef: [
        { id: 'abalado', nome: 'Abalado', nivel: 'Fraca', grupo: 'Mental', efeito: ' -1 em jogadas de ataque e testes de perícia', atk: -1, pericia: -1, def: 0, tr: 0 },
        { id: 'amedrontado', nome: 'Amedrontado', nivel: 'Média', grupo: 'Mental', efeito: ' -3 em jogadas de ataque e testes de perícia', atk: -3, pericia: -3, def: 0, tr: 0 },
        { id: 'aterrorizado', nome: 'Aterrorizado', nivel: 'Forte', grupo: 'Mental', efeito: ' Não pode se aproximar da criatura que infligiu', atk: 0, pericia: 0, def: 0, tr: 0 },
        { id: 'confuso', nome: 'Confuso', nivel: 'Média', grupo: 'Mental', efeito: ' -4 em Fortitude e Atletismo; movimento aleatório', atk: 0, pericia: 0, def: 0, tr: -4 },
        { id: 'enfeitiçado', nome: 'Enfeitiçado', nivel: 'Média', grupo: 'Mental', efeito: ' -2 em todos os testes contra quem enfeitiçou', atk: -2, pericia: -2, def: 0, tr: -2 },
        { id: 'agarrado', nome: 'Agarrado', nivel: 'Média', grupo: 'Movimento', efeito: ' Desprevenido e Imóvel', atk: 0, pericia: 0, def: -10, tr: -10 },
        { id: 'caido', nome: 'Caído', nivel: 'Fraca', grupo: 'Movimento', efeito: ' -3 ataques corpo a corpo, -3 defesa corpo a corpo, +3 defesa distância', atk: -3, pericia: 0, def: -3, tr: 0 },
        { id: 'enredado', nome: 'Enredado', nivel: 'Média', grupo: 'Movimento', efeito: ' Deslocamento reducido pela metade, -2 na defesa e ataques', atk: -2, pericia: 0, def: -2, tr: 0 },
        { id: 'imóvel', nome: 'Imóvel', nivel: 'Forte', grupo: 'Movimento', efeito: ' Não pode usar ações de movimento', atk: 0, pericia: 0, def: 0, tr: 0 },
        { id: 'lento', nome: 'Lento', nivel: 'Média', grupo: 'Movimento', efeito: ' Todo movimento reduzido pela metade', atk: 0, pericia: 0, def: 0, tr: 0 },
        { id: 'cego', nome: 'Cego', nivel: 'Forte', grupo: 'Sensorial', efeito: ' Surpreendido e Lento, -5 em Percepção, 50% de desvio', atk: 0, pericia: -5, def: 0, tr: 0 },
        { id: 'desorientado', nome: 'Desorientado', nivel: 'Fraca', grupo: 'Sensorial', efeito: ' Não pode usar reação contra próxima ação ofensividade', atk: 0, pericia: 0, def: 0, tr: 0 },
        { id: 'desprevenido', nome: 'Desprevenido', nivel: 'Fraca', grupo: 'Sensorial', efeito: ' -3 na Defesa e testes de Reflexos', atk: 0, pericia: 0, def: -3, tr: -3 },
        { id: 'surdo', nome: 'Surdo', nivel: 'Média', grupo: 'Sensorial', efeito: ' Falha em testes de audição, -5 em Iniciativa', atk: 0, pericia: 0, def: 0, tr: 0 },
        { id: 'surpreso', nome: 'Surpreso', nivel: 'Especial', grupo: 'Sensorial', efeito: ' Desprevenido, não pode reagir', atk: 0, pericia: 0, def: -3, tr: 0 },
        { id: 'exposto', nome: 'Exposto', nivel: 'Forte', grupo: 'Vulnerabilidade', efeito: ' +4 em ataques contra, +dano igual ao nível do atacante', atk: 0, pericia: 0, def: 0, tr: 0 },
        { id: 'fragilizado', nome: 'Fragilizado', nivel: 'Forte', grupo: 'Vulnerabilidade', efeito: ' RD e resistências anulados', atk: 0, pericia: 0, def: 0, tr: 0 },
        { id: 'paralisado', nome: 'Paralisado', nivel: 'Extrema', grupo: 'Incapacitação', efeito: ' Não pode agir, -10 defesa, falha Reflexos', atk: 0, pericia: 0, def: -10, tr: -10 },
        { id: 'inconsciente', nome: 'Inconsciente', nivel: 'Extrema', grupo: 'Incapacitação', efeito: ' Não pode agir, caído, falha Reflexos, acertos críticos', atk: 0, pericia: 0, def: -10, tr: 0 },
        { id: 'atordoado', nome: 'Atordoado', nivel: 'Extrema', grupo: 'Incapacitação', efeito: ' Desprevenido, não pode realizar ações ou reações', atk: 0, pericia: 0, def: -3, tr: 0 },
        { id: 'condenado', nome: 'Condenado', nivel: 'Média', grupo: 'Física', efeito: ' Custo de PE aumentado em 1', atk: 0, pericia: 0, def: 0, tr: 0 },
        { id: 'engasgando', nome: 'Engasgando', nivel: 'Média', grupo: 'Física', efeito: ' Mudo, precisa segurar o ar', atk: 0, pericia: 0, def: 0, tr: 0 },
        { id: 'enjoado', nome: 'Enjoado', nivel: 'Média', grupo: 'Física', efeito: ' Não pode converter ações na Hierarquia', atk: 0, pericia: 0, def: 0, tr: 0 },
        { id: 'envenenado', nome: 'Envenenado', nivel: 'Média', grupo: 'Física', efeito: ' -2 em ataques, testes de resistência e perícia', atk: -2, pericia: -2, def: 0, tr: -2 },
        { id: 'sofrendo', nome: 'Sofrendo', nivel: 'Leve', grupo: 'Física', efeito: ' -5 em concentração e Prestidigitação para rituais, -3m movimento', atk: 0, pericia: -5, def: 0, tr: 0 }
    ],
    ...
    initCondicoes() {
        ...
        container.innerHTML = this.condicoesRef.map(c => `
            <button onclick="app.toggleCondicao('${c.id}')" class="btn-condicao" data-id="${c.id}" style="
            ...
            " onmouseover="this.style.borderColor='var(--accent)'" onmouseout="this.style.borderColor='var(--border)'">
                <div style="font-weight: bold; color: var(--accent);">${c.nome}</div>
                <div style="font-size: 0.65rem; color: #888;">${c.nivel} • ${c.grupo}</div>
                <div style="font-size: 0.6rem; color: #666; margin-top: 3px;">${c.efeito}</div>
            </button>
        `).join('');
    },

    toggleCondicao(condId) {
        const char = this.characters[this.activeIdx];
        if (!char.condicoes) char.condicoes = [];
        
        const idx = char.condicoes.indexOf(condId);
        if (idx > -1) {
            char.condicoes.splice(idx, 1);
        } else {
            char.condicoes.push(condId);
        }
        
        this.renderCondicoesAtivas();
        this.updateCondicaoMods();
        this.saveData();
    },

    renderCondicoesAtivas() {
        const char = this.characters[this.activeIdx];
        const container = document.getElementById('condicoes-ativas');
        const msg = document.getElementById('sem-condicoes-msg');
        
        if (!char.condicoes || char.condicoes.length === 0) {
            container.innerHTML = '<p style="text-align: center; color: #888; font-size: 0.85rem;">Nenhuma condição ativa</p>';
            return;
        }
        
        container.innerHTML = char.condicoes.map(condId => {
            const cond = this.condicoesRef.find(c => c.id === condId);
            if (!cond) return '';
            return `
                <div style="display: flex; justify-content: space-between; align-items: center; padding: 8px; background: var(--bg); border-radius: 6px; margin-bottom: 6px; border-left: 3px solid var(--hp);">
                    <div>
                        <span style="color: var(--accent); font-weight: bold; font-size: 0.9rem;">${cond.nome}</span>
                        <span style="color: #888; font-size: 0.7rem; margin-left: 8px;">${cond.nivel}</span>
                    </div>
                    <button onclick="app.toggleCondicao('${condId}')" style="background: transparent; border: none; color: var(--hp); cursor: pointer; font-size: 1rem;">✕</button>
                </div>
            `;
        }).join('');
    ...
    updateCondicaoMods() {
        ...
        char.condicaoMods = { atk: 0, pericia: 0, def: 0, tr: 0 };
        
        if (char.condicoes) {
            char.condicoes.forEach(condId => {
                const cond = this.condicoesRef.find(c => c.id === condId);
                if (cond) {
                    char.condicaoMods.atk += (cond.atk || 0);
                    char.condicaoMods.pericia += (cond.pericia || 0);
                    char.condicaoMods.def += (cond.def || 0);
                    char.condicaoMods.tr += (cond.tr || 0);
                }
            });
        }
        
        const mods = char.condicaoMods;
        let msg = '';
        if (mods.atk !== 0) msg += ` Atk: ${mods.atk > 0 ? '+' : ''}${mods.atk}`;
        if (mods.pericia !== 0) msg += ` Perícia: ${mods.pericia > 0 ? '+' : ''}${mods.pericia}`;
        if (mods.def !== 0) msg += ` DEF: ${mods.def > 0 ? '+' : ''}${mods.def}`;
        if (mods.tr !== 0) msg += ` TR: ${mods.tr > 0 ? '+' : ''}${mods.tr}`;
        
        if (msg) {
            this.showToast('Condições ativas: ' + msg, 'info');
        }
    },

    openShikigamiPanel() {
        const char = this.characters[this.activeIdx];
        const container = document.getElementById('shikigami-panel-list');
        
        ui.openModal('modal-invocar-shikigami');
        
        if (!char.invocacoes || char.invocacoes.length === 0) {
            container.innerHTML = '<p style="text-align:center; color:#888; padding: 20px;">Nenhum Shikigami criado. Vá para a tela de Invocações para criar.</p>';
            return;
        }
...
container.innerHTML = char.invocacoes.map((inv, i) => {
            const grau = grauNome[inv.grau] || '4º';
            const isInvocado = inv.pv > 0;
            return `
                <div style="background: var(--input); padding: 12px; border-radius: 8px; margin-bottom: 10px; border: 1px solid ${isInvocado ? 'var(--accent)' : 'var(--border)'};">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                        <div>
                            <span style="color: var(--accent); font-weight: bold; font-size: 1rem;">${inv.nome}</span>
                            <span style="color: var(--hp); font-size: 0.75rem; margin-left: 8px;">${grau}</span>
                            ${isInvocado ? '<span style="background: var(--accent); color: white; padding: 2px 6px; border-radius: 3px; font-size: 0.65rem; margin-left: 5px;">INVOCADO</span>' : ''}
                        </div>
                        <button class="delete-btn" onclick="app.removeInvocacao(${i}); app.openShikigamiPanel()">✕</button>
                    </div>
                    
                    <div style="display: flex; gap: 10px; align-items: center; margin-bottom: 8px;">
                        <span style="background: var(--hp); color: white; padding: 4px 8px; border-radius: 4px; font-size: 0.75rem;">PV: <input type="number" id="invoca-pv-${i}" value="${inv.pv}" style="width: 40px; background: var(--hp); border: none; color: white; text-align: center; font-weight: bold;" onchange="app.updateInvocaPV(${i}, this.value)"> / ${inv.pvMax}</span>
                        <span style="background: var(--accent); color: white; padding: 4px 8px; border-radius: 4px; font-size: 0.75rem;">CD: ${inv.cd || 10}</span>
                        ${inv.atkTreinado ? `<span style="background: var(--hp); color: white; padding: 4px 8px; border-radius: 4px; font-size: 0.75rem;">ATK: ${inv.atkTreinado}</span>` : ''}
                        ${inv.trTreinado ? `<span style="background: #8b5cf6; color: white; padding: 4px 8px; border-radius: 4px; font-size: 0.75rem;">TR: ${inv.trTreinado}</span>` : ''}
                    </div>
                    
                    <div style="display: flex; gap: 8px; flex-wrap: wrap; margin-top: 8px; border-top: 1px solid var(--border); padding-top: 8px;">
                        <button class="btn-roll" onclick="app.toggleInvocacao(${i})" style="${isInvocado ? 'background: var(--hp);' : 'background: var(--accent);'}">${isInvocado ? '❌ Desinvocar' : '✨ Invocar'}</button>
                        ${isInvocado ? `
                        <button class="btn-roll" onclick="app.rollInvocaAtk(${i}, 'FOR')">⚔️ ATK</button>
                        ` : ''}
                    </div>
                    
                    ${inv.acoes && inv.acoes.length > 0 && isInvocado ? `
                    <div style="display: flex; gap: 5px; flex-wrap: wrap; margin-top: 8px;">
                        ${inv.acoes.map((acao, ai) => `
                            <button class="btn-roll" style="background: var(--card); border: 1px solid var(--accent); font-size: 0.7rem;" onclick="app.rollInvocaAcao(${i}, ${ai})">${acao}</button>
                        `).join('')}
                    </div>
                    ` : ''}
                </div>`;
        ...
        if (char.invocacoes && char.invocacoes.length > 0) {
            container.innerHTML += `
                <button class="btn-main" onclick="ui.showScreen('invocacoes-screen'); ui.closeModal('modal-invocar-shikigami')" style="margin-top: 15px; width: 100%;">➕ Criar Nova Invocação</button>
            `;
        }
    },

    updateInvocaPV(idx, value) {
        const char = this.characters[this.activeIdx];
        let pv = parseInt(value) || 0;
        if (pv < 0) pv = 0;
        if (pv > char.invocacoes[idx].pvMax) pv = char.invocacoes[idx].pvMax;
        char.invocacoes[idx].pv = pv;
        this.saveData();
        this.renderInvocacoes();
    },

    adjustInvocaPV(idx, amount) {
        const char = this.characters[this.activeIdx];
        const inv = char.invocacoes[idx];
        inv.pv += amount;
        if (inv.pv < 0) inv.pv = 0;
        if (inv.pv > inv.pvMax) inv.pv = inv.pvMax;
        this.saveData();
        this.renderInvocacoes();
    },
    ...
    toggleInvocacao(idx) {
        ...
        if (inv.pv > 0) {
            inv.pv = 0;
            this.showToast(`${inv.nome} foi desinvocado!`, 'info');
        } else {
            inv.pv = inv.pvMax || 10;
            this.showToast(`${inv.nome} foi invocado!`, 'success');
        }
    ...
    rollInvocaDano(idx) {
        ...
        toast.style.borderColor = 'var(--accent)';
        toast.innerHTML = `
            <div class="toast-title">${invoca.nome} - Teste de CD</div>
            <div class="toast-value" style="color:var(--accent); font-size:2rem;">${total}</div>
            <div class="toast-math">D20 (${d20}) + ${cd}</div>
        `;
        document.getElementById('toast-container').appendChild(toast);
        setTimeout(() => toast.remove(), 5000);
    },
    ...
    rollInvocaAtk(idx, attr) {
        ...
        // Calculo: Mod do atributo + metade do nivel + bonus de treinamento
        const modAttr = Math.floor((invoca.attrs[attr] - 10) / 2);
        const halfLvl = Math.floor(lvl / 2);
        const totalMod = modAttr + halfLvl + treino;
        
        this.rollDice(attr + ' ' + invoca.nome, totalMod);
    },
    ...
    rollInvocaPericia(idx, periciaNome) {
        ...
        this.rollDice(periciaNome + ' (' + invoca.nome + ')', totalMod);
    },

    parseDano(danoStr, attrMods = {}) {
        try {
            const dmgStr = String(danoStr || '').toUpperCase().trim();
            if (!dmgStr) return { total: 0, rolls: [], details: [] };
            
            const results = { total: 0, rolls: [], details: [] };
            
            // Regex para múltiplos formatos: "1d6+1d4+5+DEX" ou "2d6+3" ou "1d20+INT"
            const diceRegex = /(\d+)D(\d+)/gi;
            let match;
            let lastIndex = 0;
            
            // Encontrar todos os dados
            while ((match = diceRegex.exec(dmgStr)) !== null) {
                if (lastIndex < match.index) {
                    const fixedPart = dmgStr.slice(lastIndex, match.index).trim();
                    const fixedMatch = fixedPart.match(/-?\d+/);
                    if (fixedMatch) {
                        results.total += parseInt(fixedMatch[0]) || 0;
                    }
                }
                ...
                results.total += diceTotal;
                results.details.push(`${numDice}d${faces}`);
                lastIndex = match.index + match[0].length;
            }
            
            // Processar valores fixos após os dados
            const afterDice = dmgStr.slice(lastIndex).trim();
            if (afterDice) {
                const fixedMatches = afterDice.match(/-?\d+/g);
                if (fixedMatches) {
                    fixedMatches.forEach(f => {
                        results.total += parseInt(f) || 0;
                    });
                }
                
                const attrMatches = afterDice.match(/[A-Z]{2,3}/g);
                if (attrMatches) {
                    attrMatches.forEach(attr => {
                        if (attrMods[attr] !== undefined) {
                            results.total += attrMods[attr];
                        }
                    });
                }
            }
            ...
            console.error('parseDano error:', e);
            return { total: 0, rolls: [], details: [] };
        }
    },
    ...
    rollInvocaDano(idx) {
        const char = this.characters[this.activeIdx];
        const invoca = char.invocacoes[idx];
        const customDano = invoca.dano;
        
        if (customDano && customDano.trim() !== '') {
            const attrMods = {
                FOR: Math.floor((invoca.attrs.FOR - 10) / 2),
                DEX: Math.floor((invoca.attrs.DEX - 10) / 2),
                CON: Math.floor((invoca.attrs.CON - 10) / 2),
                INT: Math.floor((invoca.attrs.INT - 10) / 2),
                SAB: Math.floor((invoca.attrs.SAB - 10) / 2),
                PRE: Math.floor((invoca.attrs.PRE - 10) / 2)
            };
            
            const result = this.parseDano(customDano, attrMods);
            
            const toast = document.createElement('div');
            toast.className = 'dice-toast fade-in';
            toast.style.borderColor = 'var(--hp)';
            toast.innerHTML = `
                <div class="toast-title">Dano ${invoca.nome}</div>
                <div class="toast-value" style="color:var(--hp); font-size:2rem;">${result.total}</div>
                <div class="toast-math">${result.details.join('+') || customDano} (${result.rolls.join(', ')})</div>
            `;
            document.getElementById('toast-container').appendChild(toast);
            setTimeout(() => toast.remove(), 5000);
            return;
        ...
        this.showToast('Defina o dano da invocação no modal de edição', 'info');
    },
    ...
    resetInvocacaoForm() {
        document.getElementById('invoca-tipo').value = 'shikigami';
        document.getElementById('invoca-grau').value = 4;
        document.getElementById('invoca-nome').value = '';
        document.getElementById('invoca-desc').value = '';
        document.getElementById('invoca-pv').value = 10;
        document.getElementById('invoca-def').value = 10;
        document.getElementById('invoca-cd').value = 10;
        document.getElementById('invoca-desloc').value = '9m';
        document.getElementById('invoca-dano').value = '';
        document.getElementById('invoca-atk-treinado').value = '';
        document.getElementById('invoca-tr-treinado').value = '';
        document.getElementById('invoca-edit-idx').value = '';
        ...
        ['for', 'dex', 'con', 'int', 'sab', 'pre'].forEach(attr => {
            document.getElementById('invoca-' + attr).value = 8;
        });
    },

    openAddInvocacaoModal() {
        this.resetInvocacaoForm();
        this.renderInvocaSkillsInModal();
        this.calcularInvocacaoStats();
        ui.openModal('modal-add-invocacao');
    },

    saveData() {
        // Se estiver editando a ficha de um player (modo mestre), salvar no Firebase do player
        if (this.modoEdicaoPlayer && this.modoEdicaoPlayer.memberUid) {
            const char = this.characters[this.activeIdx];
            if (char) {
                const { memberUid, charIdx } = this.modoEdicaoPlayer;
                db.ref('usuarios/' + memberUid + '/fichas/' + charIdx).set(char).then(() => {
                    this.showToast('Ficha do player atualizada!', 'success');
                }).catch(err => {
                    console.error('Erro ao salvar ficha do player:', err);
                });
            }
        ...
        // Modo normal - salvar localmente
        localStorage.setItem('jjk_chars', JSON.stringify(this.characters));
        
        // Sempre salvar no Firebase quando há personagem ativo
        if (this.userUid && this.characters && this.characters.length > 0) {
    ...
    handleImageUpload(e) {
        ...
        if (file && this.activeIdx !== null) {
            const reader = new FileReader();
            reader.onload = ev => {
                document.getElementById('foto-tipo-edicao').value = 'ficha';
                document.getElementById('foto-temp-data').value = ev.target.result;
                document.getElementById('foto-zoom').value = 1;
                document.getElementById('foto-tamanho').value = 100;
                document.getElementById('foto-preview-img').src = ev.target.result;
...
app.carregarMinhasCampanhas = function() {
    if (this.campanhaListInterval) {
        clearInterval(this.campanhaListInterval);
    }
    ...
    const carregar = () => {
        ...
        db.ref('campanhas').orderByChild('membros').limitToLast(20).once('value').then(snapshot => {
            if (!snapshot.exists()) {
                if (container) container.innerHTML = '<div style="text-align: center; color: #888; padding: 20px;">Nenhuma campanha encontrada</div>';
                return;
            }
            ...
            snapshot.forEach(child => {
                ...
                const membros = Array.isArray(c.membros) ? c.membros : Object.values(c.membros || {});
                const isMembro = membros.includes(uid);
                if (!isMembro) return;
                ...
                html += `
                    <div class="char-slot" onclick="app.acessarCampanha('${child.key}')">
                        <div class="slot-info">
                            <div class="slot-name">${c.nome || 'Sem nome'}</div>
            ...
            if (!container) return;
            container.innerHTML = html || '<div style="text-align: center; color: #888; padding: 20px;">Nenhuma campanha encontrada</div>';
        }).catch(() => {});
    };
    
    carregar();
    this.campanhaListInterval = setInterval(carregar, 10000);
};
...
app.acessarCampanha = function(campanhaId) {
    ...
    const nomeEl = document.getElementById('campanha-interna-nome');
    const nomeEditarEl = document.getElementById('campanha-nome-editar');
    const mestreActionsEl = document.getElementById('campanha-mestre-actions');
    const escudoMestreEl = document.getElementById('btn-escudo-mestre');
    ...
    db.ref('campanhas/' + campanhaId).once('value').then(snapshot => {
        ...
        if (nomeEditarEl) nomeEditarEl.value = c.nome;
        
        const uid = this.userUid || 'local';
        const isMestre = c.mestre === uid;
        
        if (mestreActionsEl) mestreActionsEl.style.display = isMestre ? 'block' : 'none';
        if (escudoMestreEl) escudoMestreEl.style.display = isMestre ? 'block' : 'none';
        
        const excluirBtn = document.getElementById('btn-excluir-campanha');
        if (excluirBtn) excluirBtn.style.display = isMestre ? 'block' : 'none';
        ...
        console.error('Erro ao acessar campanha:', err);
        this.showToast('Erro ao acessar campanha', 'error');
    });
};
...
app.escucharFichasCampanha = function(campanhaId) {
    ...
    if (this.campanhaPollingInterval) {
        clearInterval(this.campanhaPollingInterval);
    }
    
    const self = this;
    const panel = document.getElementById('escudo-mestre-panel');
    const rolagensContainer = document.getElementById('rolagens-campanha-list');
    
    const polling = () => {
        if (!self.campanhaAtiva) {
            clearInterval(self.campanhaPollingInterval);
            return;
        }
        
        db.ref('campanhas/' + campanhaId + '/fichas').limitToLast(10).once('value').then(snapshot => {
            if (panel && panel.style.display === 'block' && snapshot.exists()) {
                self.renderizarFichasCampanha(campanhaId);
            }
        }).catch(() => {});
        
        if (rolagensContainer && rolagensContainer.style.display !== 'none') {
            db.ref('campanhas/' + campanhaId + '/rolagens').limitToLast(5).once('value').then(snap => {
                if (snap.exists()) {
                    self.mostrarRolagensCampanha(campanhaId, true);
                }
            }).catch(() => {});
        }
    };
    ...
    polling();
    this.campanhaPollingInterval = setInterval(polling, 5000);
};

// Função para o mestre acessar a ficha de um player
app.acessarFichaPlayer = function(memberUid, charIdx) {
    if (!this.campanhaAtiva) return this.showToast('Nenhuma campanha ativa', 'error');
    
    const campanhaId = this.campanhaAtiva.id;
    
    // Buscar a ficha do player no Firebase
    db.ref('usuarios/' + memberUid + '/fichas/' + charIdx).once('value').then(snapshot => {
        const char = snapshot.val();
        if (!char) {
            this.showToast('Ficha não encontrada', 'error');
            return;
        }
        
        // Armazenar temporariamente a ficha do player para edição
        this.fichaPlayerEdit = { memberUid, charIdx, char };
        
        // Mostrar toast confirmando acesso
        this.showToast(`Acessando ficha de ${char.name || 'Jogador'} como mestre`, 'info');
        
        // Carregar a ficha para edição
        if (!this.characters) this.characters = [];
        
        // Se já tem personagem ativo, salvar antes
        if (this.activeIdx !== null) {
            this.saveData();
        }
        
        // Temporariamente substituir o personagem atual pela ficha do player
        this.characters.push(char);
        this.activeIdx = this.characters.length - 1;
        
        // Guardar que está em modo edição de player
        this.modoEdicaoPlayer = { memberUid, charIdx, originalActiveIdx: this.activeIdx };
        
        // Abrir a ficha
        this.openSheet(this.activeIdx);
        
        this.showToast('Você está editando a ficha do player (modo mestre)', 'info');
    }).catch(err => {
        console.error('Erro ao acessar ficha do player:', err);
        this.showToast('Erro ao acessar ficha', 'error');
    });
};

// Função para voltar à própria ficha após edição
app.voltarFichaOriginal = function() {
    if (!this.modoEdicaoPlayer) return;
    
    const { originalActiveIdx } = this.modoEdicaoPlayer;
    
    // Remover a ficha temporária
    if (this.characters && this.characters.length > originalActiveIdx) {
        this.characters.splice(originalActiveIdx, 1);
    }
    
    this.activeIdx = originalActiveIdx;
    this.modoEdicaoPlayer = null;
    this.fichaPlayerEdit = null;
    
    // Recarregar a ficha original
    this.openSheet(this.activeIdx);
    this.showToast('Voltou para sua ficha original', 'info');
};

app.escucharPlayersCampanha = function(campanhaId) {
    if (!campanhaId) return;
    
    if (this.campanhaMembrosListener) {
        try { db.ref('campanhas/' + campanhaId + '/membros').off('value', this.campanhaMembrosListener); } catch(e) {}
    }
    
    const self = this;
    const playersPanel = document.getElementById('players-campanha-panel');
    
    this.campanhaMembrosListener = db.ref('campanhas/' + campanhaId + '/membros').on('value', snapshot => {
        if (playersPanel && playersPanel.style.display === 'block') {
            self.mostrarPlayersCampanha();
        }
    });
};
...
app.escucharStatusJogadoresCampanha = function(campanhaId) {
    ...
    if (this.campanhaStatusListener) {
        try { db.ref('campanhas/' + campanhaId + '/fichas').off('value', this.campanhaStatusListener); } catch(e) {}
    }
    
    const self = this;
    this.campanhaStatusListener = db.ref('campanhas/' + campanhaId + '/fichas').on('value', snapshot => {
        const fichasData = snapshot.val() || {};
        
        Object.entries(fichasData).forEach(([memberUid, charList]) => {
            if (!Array.isArray(charList)) return;
            
            charList.forEach((charIdx) => {
                if (self.activeIdx !== null && self.characters && self.characters[self.activeIdx]) {
                    const char = self.characters[self.activeIdx];
                }
            });
        });
        ...
        if (statusPanel && statusPanel.style.display === 'block') {
            self.renderizarStatusJogadores(campanhaId);
        }
    });
};

app.renderizarFichasCampanha = function(campanhaId) {
    const container = document.getElementById('fichas-campanha-container');
    if (!container) return;
    
    const maintenant = Date.now();
    if (this.fichasCampanhaCache && this.fichasCampanhaTime > maintenant - 8000) {
        if (this.fichasCampanhaCacheHtml) {
            container.innerHTML = this.fichasCampanhaCacheHtml;
        }
        return;
    }
    ...
    db.ref('campanhas/' + campanhaId).once('value').then(campSnap => {
                ...
                Promise.all(memberPromises).then(() => {
                    for (const [memberUid, charIndices] of Object.entries(fichasData)) {
                        ...
                        charIndices.forEach(charIdx => {
                            ...
                            // Primeiro busca nos dados locais do app
                            if (memberUid === currentUid && this.characters[charIdx]) {
                                char = this.characters[charIdx];
                            }
                            
                            // Se não achou, busca nos dados do Firebase do membro
                            if (!char && memberFichasData[memberUid]) {
                                char = memberFichasData[memberUid][charIdx];
                            }
                            
                            // Se ainda não achou, busca direto no Firebase
                            if (!char) {
                                db.ref('usuarios/' + memberUid + '/fichas/' + charIdx).once('value').then(snap => {
                                    char = snap.val();
                                    if (char) {
                                        renderizarCard(char, memberUid, charIdx, currentUid, isMestre);
                                    }
                                });
                                return;
                            }
                    ...
                    const finalHtml = html || '<div style="text-align: center; color: #888; padding: 20px;">Nenhum feiticeiro encontrado</div>';
                    document.getElementById('fichas-campanha-container').innerHTML = finalHtml;
                });
            }
            
            function renderizarCard(char, memberUid, charIdx, currentUid, isMestre) {
                ...
                // Botão para o mestre acessar a ficha do player
                const accessBtn = isMestre ? `<button class="btn-roll" onclick="app.acessarFichaPlayer('${memberUid}', ${charIdx})" style="margin-left: 5px; padding: 5px 10px; font-size: 0.7rem; background: var(--accent);">👁️ Acessar</button>` : '';
...
app.limparListenersCampanha = function() {
    if (this.campanhaPollingInterval) {
        clearInterval(this.campanhaPollingInterval);
        this.campanhaPollingInterval = null;
    }
    if (this.campanhaListInterval) {
        clearInterval(this.campanhaListInterval);
        this.campanhaListInterval = null;
    }
...
app.sairDaCampanha = function() {
    ...
    db.ref('campanhas/' + campanhaId + '/membros').once('value').then(snapshot => {
        ...
        if (!Array.isArray(membros)) {
            membros = Object.values(membros);
        }
        
        membros = membros.filter(m => m !== uid);
        
        return db.ref('campanhas/' + campanhaId + '/membros').set(membros);
    }).then(() => {
        this.campanhaAtiva = null;
        ui.showScreen('campaigns-screen');
        this.showToast('Você saiu da campanha!', 'success');
        ...
        this.showToast('Erro ao sair da campanha', 'error');
    });
};
...
app.excluirCampanha = function() {
    ...
    if (!confirm('CONFIRMAÇÃO FINAL: Isso excluirá a campanha permanentemente!')) return;
...
app.mostrarRolagensCampanha = function(campanhaId) {
    const rolagensList = document.getElementById('rolagens-campanha-list');
    const playersPanel = document.getElementById('players-campanha-panel');
    const btnRolagens = document.getElementById('btn-rolagens-campanha');
    const btnPlayers = document.getElementById('btn-players-campanha');
    
    if (rolagensList) rolagensList.style.display = 'block';
    if (playersPanel) playersPanel.style.display = 'none';
    if (btnRolagens) btnRolagens.classList.add('active');
    if (btnPlayers) btnPlayers.classList.remove('active');
    
    if (campanhaId || (this.campanhaAtiva && this.campanhaAtiva.id)) {
        this.escucharRolagensCampanha(campanhaId || this.campanhaAtiva.id);
    }
};
...
app.renderizarStatusJogadores = function(campanhaId) {
    ...
    const listEl = document.getElementById('status-jogadores-list');
    if (!listEl) return;
    
    listEl.innerHTML = '<div style="text-align:center; color:#888; padding:20px;">Carregando...</div>';
    
    db.ref('campanhas/' + campanhaId + '/fichas').once('value').then(snapshot => {
        const fichasData = snapshot.val() || {};
        
        if (Object.keys(fichasData).length === 0) {
            listEl.innerHTML = '<div style="text-align:center; color:#888; padding:20px;">Nenhum player na campanha</div>';
            return;
        }
        ...
        Object.entries(fichasData).forEach(([memberUid, charList]) => {
            ...
            charList.forEach(charIdx => {
                const p = db.ref('usuarios/' + memberUid + '/fichas/' + charIdx).once('value').then(charSnap => {
        ...
        Promise.all(promises).then(results => {
            html += results.filter(r => r).join('') + '</div>';
            listEl.innerHTML = html;
        });
    }).catch(err => {
        console.error('Erro ao carregar status:', err);
        listEl.innerHTML = '<div style="text-align:center; color:#f44; padding:20px;">Erro ao carregar</div>';
    });
};

app.editarStatusJogador = function(memberUid, charIdx, tipo, valor) {
    if (!memberUid || charIdx === undefined || charIdx === null) return;
    
    const hpRef = db.ref('usuarios/' + memberUid + '/fichas/' + charIdx + '/hp');
    const peRef = db.ref('usuarios/' + memberUid + '/fichas/' + charIdx + '/pe');
    
    if (tipo === 'hp') {
        hpRef.once('value').then(snapshot => {
            const hp = snapshot.val() || { curr: 10, max: 10 };
            const newCurr = Math.max(0, Math.min(hp.max || 10, (hp.curr || 0) + valor));
            return hpRef.set({ curr: newCurr, max: hp.max || 10, temp: hp.temp || 0 });
        }).then(() => {
            this.showToast('HP atualizado!', 'success');
        }).catch(err => {
            console.error('Erro ao atualizar HP:', err);
        });
    } else if (tipo === 'pe') {
        peRef.once('value').then(snapshot => {
            const pe = snapshot.val() || { curr: 4, max: 4 };
            const newCurr = Math.max(0, Math.min(pe.max || 4, (pe.curr || 0) + valor));
            return peRef.set({ curr: newCurr, max: pe.max || 4, temp: pe.temp || 0 });
        }).then(() => {
            this.showToast('PE atualizado!', 'success');
        }).catch(err => {
            console.error('Erro ao atualizar PE:', err);
        });
    }
};
...
app.escucharRolagensCampanha = function(campanhaId) {
    ...
    if (this.campanhaRolagensListener) {
        ...
        this.campanhaRolagensListener = null;
    }
