const app = {
    ...
    autoCalculate() {
        ...
        // Iniciativa = 1d20 + DEX mod + (nível/2) + treino
        const initMod = mods.DEX + metNivel + treino;
    ...
    addCustomFeitico() {
        ...
        const feiticoData = nome + '|' + desc + '|' + dano + '|' + danoAttr + '|' + tipo;
    ...
    editarFeitico(idx) {
        ...
        document.getElementById('feitico-dano').value = parts[2] || '';
        document.getElementById('feitico-tipo').value = parts[4] || 'ataque';
...
editarHabilidade(idx) {
    ...
    editarInvocacao(idx) {
        ...
        if (inv.attrs) {
            document.getElementById('invoca-for').value = inv.attrs.FOR || 10;
            document.getElementById('invoca-dex').value = inv.attrs.DEX || 10;
            document.getElementById('invoca-con').value = inv.attrs.CON || 10;
            document.getElementById('invoca-int').value = inv.attrs.INT || 10;
            document.getElementById('invoca-sab').value = inv.attrs.SAB || 10;
            document.getElementById('invoca-pre').value = inv.attrs.PRE || 10;
        }
...
updateCondicoes() {
            ...
            return `<div class="invoca-card" style="background:var(--card); border:1px solid var(--border); border-radius:8px; padding:15px; margin-bottom:10px; position:relative; animation:slideIn 0.3s ease;">
                ...
                <div style="display:flex; gap:15px; flex-wrap:wrap; margin-top:10px;">
                    ...
                    <div style="background:var(--input); padding:5px 10px; border-radius:4px; font-size:0.75rem;">
                        <span style="color:#888;">DEF:</span> <span style="color:var(--energy); font-weight:bold;">${inv.def}</span>
                    </div>
                    <div style="background:var(--input); padding:5px 10px; border-radius:4px; font-size:0.75rem;">
    ...
    editInvocacao(idx) {
        ...
        document.getElementById('invoca-desc').value = inv.desc || '';
        document.getElementById('invoca-pv').value = inv.pv || 10;
        document.getElementById('invoca-def').value = inv.def || 10;
        document.getElementById('invoca-cd').value = inv.cd || 10;
        document.getElementById('invoca-desloc').value = inv.desloc || '9m';
        document.getElementById('invoca-dano').value = inv.dano || '';
        document.getElementById('invoca-tr-treinado').value = inv.trTreinado || '';
        ...
        if (inv.pericias) {
            document.querySelectorAll('#invoca-skill-list .invoca-pericia-checkbox').forEach(cb => {
                cb.checked = inv.pericias.includes(cb.value);
            });
        }
    ...
    openShikigamiPanel() {
        ...
        // Primeiro atualiza a lista no dashboard
        this.renderShikigamisResumo();
